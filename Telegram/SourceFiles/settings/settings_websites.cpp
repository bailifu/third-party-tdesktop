...
void InfoBox(not_null<Ui::GenericBox *> box, const EntryData &data,
  ...
  const auto userpic =
      box->addRow(object_ptr<Ui::CenterWrap<Ui::UserpicButton>>(
                      box, object_ptr<Ui::UserpicButton>(
                               box, data.bot, st::websiteBigUserpic)),
  ...
  const auto name = Ui::CreateChild<Ui::FlatLabel>(
      nameWrap, rpl::single(data.bot->name()), st::sessionBigName);
  ...
  const auto domain = Ui::CreateChild<Ui::FlatLabel>(
      domainWrap, rpl::single(data.domain), st::sessionDateLabel);
  rpl::combine(domainWrap->widthValue(), domain->widthValue()) |
      rpl::start_with_next(
          [=](int outer, int inner) { domain->move((outer - inner) / 2, 0); },
          domain->lifetime());
  ...
  AddSessionInfoRow(container, tr::lng_sessions_browser(),
...
void Row::update(const EntryData &data) {
  _data = data;
  setCustomStatus(JoinNonEmpty({_data.domain, _data.browser, _data.platform}));
...
QString Row::generateName() { return _data.bot->name(); }

QString Row::generateShortName() { return _data.bot->shortName(); }
...
QRect Row::elementGeometry(int element, int outerWidth) const {
  switch (element) {
    case 1: {
      return QRect(st::websiteListItem.namePosition.x(), st::websiteLocationTop,
...
void Row::elementsPaint(Painter &p, int outerWidth, bool selected,
  ...
  const auto position = geometry.topLeft() + st::sessionTerminate.iconPosition;
  ...
  _location.drawLeftElided(p, locationLeft, st::websiteLocationTop, available,
                           outerWidth);
}

class Content : public Ui::RpWidget {
 public:
  Content(QWidget *, not_null<Window::SessionController *> controller);
...
class Content::ListController final : public PeerListController,
                                      public RowDelegate,
...
Content::Content(QWidget *, not_null<Window::SessionController *> controller)
...
void Content::setupContent() {
  ...
  _inner->showRequests() |
      rpl::start_with_next(
          [=](const EntryData &data) {
            _controller->show(
                Box(InfoBox, data, [=](uint64 hash) { terminateOne(hash); }));
          },
          lifetime());
  ...
  _websites->listValue() |
      rpl::start_with_next(
          [=](const Api::Websites::List &list) { parse(list); }, lifetime());
...
void Content::shortPoll() {
  const auto left =
      kShortPollTimeout - (crl::now() - _websites->lastReceivedTime());
  if (left > 0) {
...
void Content::terminate(Fn<void(bool block)> sendRequest,
                        rpl::producer<QString> title,
                        rpl::producer<QString> text, QString blockText) {
...
void Content::terminateOne(uint64 hash) {
  ...
  auto callback = [=](bool block) {
    auto done = crl::guard(weak, [=](const MTPBool &result) {
      _data.erase(ranges::remove(_data, hash, &EntryData::hash), end(_data));
      _inner->showData(_data);
    });
    auto fail = crl::guard(weak, [=](const MTP::Error &error) {});
    _websites->requestTerminate(std::move(done), std::move(fail), hash,
                                block ? bot.get() : nullptr);
  };
  terminate(std::move(callback), tr::lng_settings_disconnect_title(),
      ...
      content
          ->add(object_ptr<Ui::SlideWrap<Ui::FlatLabel>>(
              ...
              object_ptr<Ui::FlatLabel>(
                  content, tr::lng_settings_logged_in_description(),
                  st::boxDividerLabel),
...
Content::ListController::ListController(not_null<Main::Session *> session)
    : _session(session) {}

Main::Session &Content::ListController::session() const { return *_session; }

void Content::ListController::prepare() {}

void Content::ListController::rowClicked(not_null<PeerListRow *> row) {
  _showRequests.fire_copy(static_cast<Row *>(row.get())->data());
}

void Content::ListController::rowElementClicked(not_null<PeerListRow *> row,
                                                int element) {
  if (element == 2) {
...
void Content::ListController::showData(gsl::span<const EntryData> items) {
  auto index = 0;
...
void Websites::setupContent(not_null<Window::SessionController *> controller) {
  ...
  const auto content =
      container->add(object_ptr<Content>(container, controller));
