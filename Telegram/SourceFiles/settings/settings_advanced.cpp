...
void SetupUpdate(not_null<Ui::VerticalLayout *> container) {
  ...
  rpl::combine(toggle->widthValue(), label->widthValue()) |
      rpl::start_with_next(
          [=] {
            label->moveToLeft(st::settingsUpdateStatePosition.x(),
  ...
  const auto showDownloadProgress = [=](int64 ready, int64 total) {
    texts->fire(tr::lng_settings_downloading_update(
        tr::now, lt_progress, Ui::FormatDownloadText(ready, total)));
  ...
  if (install) {
    ...
    install->toggledValue() | rpl::filter([](bool toggled) {
  ...
  Core::UpdateChecker checker;
  options->toggleOn(rpl::combine(toggle->toggledValue(),
                                 downloading->events_starting_with(
                                     checker.state() ==
                                     Core::UpdateChecker::State::Download)) |
...
void SetupSystemIntegrationContent(Window::SessionController *controller,
  ...
  if (!Platform::RunInBackground()) {
    const auto closeToTaskbar = addSlidingCheckbox(
        tr::lng_settings_close_to_taskbar(), settings->closeToTaskbar());

    const auto closeToTaskbarShown =
    ...
    closeToTaskbar->entity()->checkedChanges() | rpl::filter([=](bool checked) {
