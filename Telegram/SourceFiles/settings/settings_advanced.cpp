...
void SetupSpellchecker(not_null<Window::SessionController *> controller,
  ...
  const auto button =
      container
          ->add(object_ptr<Button>(container,
                                   isSystem
                                       ? tr::lng_settings_system_spellchecker()
                                       : tr::lng_settings_custom_spellchecker(),
                                   st::settingsButtonNoIcon))
          ->toggleOn(rpl::single(settings->spellcheckerEnabled()));
  ...
  sliding->entity()
          ->add(object_ptr<Button>(
              sliding->entity(), tr::lng_settings_auto_download_dictionaries(),
              st::settingsButtonNoIcon))
          ->toggleOn(rpl::single(settings->autoDownloadDictionaries()))
          ->toggledValue() |
  ...
  AddButtonWithLabel(sliding->entity(), tr::lng_settings_manage_dictionaries(),
  ...
  button->toggledValue() |
      rpl::start_with_next(
          [=](bool enabled) { sliding->toggle(enabled, anim::type::normal); },
          container->lifetime());
#endif  // !TDESKTOP_DISABLE_SPELLCHECK
}
...
void SetupWindowTitleContent(Window::SessionController *controller,
  ...
  const auto checkbox = [&](rpl::producer<QString> &&label, bool checked) {
    return object_ptr<Ui::Checkbox>(container, std::move(label), checked,
                                    st::settingsCheckbox);
  };
  const auto addCheckbox = [&](rpl::producer<QString> &&label, bool checked) {
    return container->add(checkbox(std::move(label), checked),
                          st::settingsCheckboxPadding);
  };
  const auto settings = &Core::App().settings();
  if (controller) {
    const auto content = [=] { return settings->windowTitleContent(); };
    const auto showChatName = addCheckbox(tr::lng_settings_title_chat_name(),
                                          !content().hideChatName);
    showChatName->checkedChanges() | rpl::filter([=](bool checked) {
    ...
    if (Core::App().domain().accountsAuthedCount() > 1) {
      const auto showAccountName = addCheckbox(
          tr::lng_settings_title_account_name(), !content().hideAccountName);
      showAccountName->checkedChanges() | rpl::filter([=](bool checked) {
    ...
    const auto showTotalUnread = addCheckbox(
        tr::lng_settings_title_total_count(), !content().hideTotalUnread);
    showTotalUnread->checkedChanges() | rpl::filter([=](bool checked) {
  ...
  if (Ui::Platform::NativeWindowFrameSupported()) {
    const auto nativeFrame =
        addCheckbox(Platform::IsWayland() ? tr::lng_settings_qt_frame()
                                          : tr::lng_settings_native_frame(),
                    Core::App().settings().nativeWindowFrame());

    nativeFrame->checkedChanges() | rpl::filter([](bool checked) {
...
void SetupSystemIntegrationContent(Window::SessionController *controller,
  ...
  const auto addSlidingCheckbox = [&](rpl::producer<QString> &&label,
                                      bool checked) {
    return container->add(object_ptr<Ui::SlideWrap<Ui::Checkbox>>(
        container, checkbox(std::move(label), checked),
        st::settingsCheckboxPadding));
  };
  ...
  if (Platform::TrayIconSupported()) {
    ...
    const auto tray =
        addCheckbox(tr::lng_settings_workmode_tray(), trayEnabled());
    const auto monochrome =
        Platform::HasMonochromeSetting()
            ? addSlidingCheckbox(tr::lng_settings_monochrome_icon(),
                                 settings->trayIconMonochrome())
            : nullptr;
    if (monochrome) {
      monochrome->toggle(tray->checked(), anim::type::instant);

      monochrome->entity()->checkedChanges() | rpl::filter([=](bool value) {
    ...
    const auto taskbar =
        Platform::SkipTaskbarSupported()
            ? addCheckbox(tr::lng_settings_workmode_window(), taskbarEnabled())
            : nullptr;

    const auto updateWorkmode = [=] {
      const auto newMode = tray->checked() ? ((!taskbar || taskbar->checked())
    ...
    tray->checkedChanges() |
        rpl::filter([=](bool checked) { return (checked != trayEnabled()); }) |
        rpl::start_with_next(
            [=](bool checked) {
    ...
    if (taskbar) {
      taskbar->checkedChanges() | rpl::filter([=](bool checked) {
  ...
  const auto warnBeforeQuit = addCheckbox(
      tr::lng_settings_mac_warn_before_quit(
          lt_text, rpl::single(Platform::ConfirmQuit::QuitKeysString())),
      settings->macWarnBeforeQuit());
  warnBeforeQuit->checkedChanges() | rpl::filter([=](bool checked) {
  ...
  const auto roundIcon =
      addCheckbox(tr::lng_settings_mac_round_icon(), enabled());
  roundIcon->checkedChanges() |
      rpl::filter([=](bool checked) { return (checked != enabled()); }) |
      rpl::start_with_next(
          [=](bool checked) {
            const auto digest =
                checked ? base::Platform::SetCustomAppIcon(IconMacRound())
                        : std::optional<uint64>();
            if (!checked) {
  ...
  const auto closeToTaskbar =
      addSlidingCheckbox(tr::lng_settings_close_to_taskbar(),
                         settings->closeBehavior() == Behavior::CloseToTaskbar);

  const auto closeToTaskbarShown = std::make_shared<rpl::variable<bool>>(false);
  settings->workModeValue() | rpl::start_with_next(
                                  [=](WorkMode workMode) {
                                    *closeToTaskbarShown =
                                        !Core::App().tray().has();
                                  },
  ...
  closeToTaskbar->entity()->checkedChanges() | rpl::map([=](bool checked) {
  ...
  if (Platform::AutostartSupported() && controller) {
    ...
    const auto autostart =
        addCheckbox(tr::lng_settings_auto_start(), cAutoStart());
    const auto minimized =
        addSlidingCheckbox(tr::lng_settings_start_min(), minimizedToggled());

    autostart->checkedChanges() |
        rpl::filter([](bool checked) { return (checked != cAutoStart()); }) |
        rpl::start_with_next(
            [=](bool checked) {
              const auto weak = base::make_weak(controller);
              cSetAutoStart(checked);
              Platform::AutostartToggle(
                  checked, crl::guard(autostart, [=](bool enabled) {
    ...
    Platform::AutostartRequestStateFromSystem(crl::guard(
        controller, [=](bool enabled) { autostart->setChecked(enabled); }));

    minimized->toggleOn(autostart->checkedValue());
    minimized->entity()->checkedChanges() | rpl::filter([=](bool checked) {
  ...
  if (Platform::IsWindows() && !Platform::IsWindowsStoreBuild()) {
    const auto sendto =
        addCheckbox(tr::lng_settings_add_sendto(), cSendToMenu());

    sendto->checkedChanges() |
        rpl::filter([](bool checked) { return (checked != cSendToMenu()); }) |
        rpl::start_with_next(
            [](bool checked) {
...
void CheckNonEmptyOptions(not_null<Window::SessionController *> controller,
                          not_null<Ui::VerticalLayout *> container, Fill fill) {
...
void SetupSystemIntegrationOptions(
    ...
    not_null<Ui::VerticalLayout *> container) {
  CheckNonEmptyOptions(controller, container, SetupSystemIntegrationContent);
}

void SetupWindowTitleOptions(not_null<Window::SessionController *> controller,
                             not_null<Ui::VerticalLayout *> container) {
  CheckNonEmptyOptions(controller, container, SetupWindowTitleContent);
}

void SetupAnimations(not_null<Window::Controller *> window,
                     not_null<Ui::VerticalLayout *> container) {
  container
      ->add(object_ptr<Button>(container, tr::lng_settings_power_menu(),
                               st::settingsButtonNoIcon))
...
void ArchiveSettingsBox(not_null<Ui::GenericBox *> box,
  ...
  container
      ...
      rpl::start_with_next(
          [=](bool toggled) {
            privacy->updateUnarchiveOnNewMessage(
                toggled                     ? Unarchive::None
                : state->folders->toggled() ? Unarchive::NotInFoldersUnmuted
  ...
  state->folders =
      inner
          ->add(object_ptr<Button>(inner, tr::lng_settings_always_in_archive(),
                                   st::settingsButtonNoIcon))
          ->toggleOn(privacy->unarchiveOnNewMessage() |
                     rpl::map(rpl::mappers::_1 != Unarchive::AnyUnmuted));
  state->folders->toggledChanges() | rpl::filter([=](bool toggled) {
    const auto current = privacy->unarchiveOnNewMessageCurrent();
...
void SetupHardwareAcceleration(not_null<Ui::VerticalLayout *> container) {
  const auto settings = &Core::App().settings();
  container
          ->add(object_ptr<Button>(container, tr::lng_settings_enable_hwaccel(),
                                   st::settingsButtonNoIcon))
          ->toggleOn(rpl::single(settings->hardwareAcceleratedVideo()))
          ->toggledValue() |
      rpl::filter([=](bool enabled) {
...
void SetupANGLE(not_null<Window::SessionController *> controller,
                not_null<Ui::VerticalLayout *> container) {
  ...
  const auto button = AddButtonWithLabel(
      container, tr::lng_settings_angle_backend(),
      rpl::single(options[backendIndex]), st::settingsButtonNoIcon);
...
void SetupOpenGL(not_null<Window::SessionController *> controller,
                 not_null<Ui::VerticalLayout *> container) {
  const auto toggles =
      container->lifetime().make_state<rpl::event_stream<bool>>();
  const auto button =
      container
          ->add(object_ptr<Button>(container, tr::lng_settings_enable_opengl(),
                                   st::settingsButtonNoIcon))
          ->toggleOn(toggles->events_starting_with_copy(
...
void SetupPerformance(not_null<Window::SessionController *> controller,
                      not_null<Ui::VerticalLayout *> container) {
...
void SetupWindowTitle(not_null<Window::SessionController *> controller,
                      not_null<Ui::VerticalLayout *> container) {
...
void SetupWindowCloseBehavior(not_null<Window::SessionController *> controller,
                              not_null<Ui::VerticalLayout *> container) {
  ...
  using Behavior = Core::Settings::CloseBehavior;
  const auto group =
      std::make_shared<Ui::RadioenumGroup<Behavior>>(settings->closeBehavior());
  const auto add = [&](Behavior value, const QString &label) {
    inner->add(object_ptr<Ui::Radioenum<Behavior>>(inner, group, value, label,
                                                   st::settingsSendType),
               st::settingsSendTypePadding);
  };

  add(Behavior::RunInBackground, tr::lng_settings_run_in_background(tr::now));
  add(Behavior::CloseToTaskbar, tr::lng_settings_close_to_taskbar(tr::now));
  add(Behavior::Quit, tr::lng_settings_quit_on_close(tr::now));
...
void SetupSystemIntegration(not_null<Window::SessionController *> controller,
                            not_null<Ui::VerticalLayout *> container) {
  AddDivider(container);
  AddSkip(container);
...
rpl::producer<QString> Advanced::title() { return tr::lng_settings_advanced(); }
