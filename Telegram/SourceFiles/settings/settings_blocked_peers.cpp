...
Blocked::Blocked(QWidget *parent,
...

  {
    ...
    _loading = base::make_unique_q<Ui::CenterWrap<>>(
        this, object_ptr<Ui::PaddingWrap<>>(
                  this,
                  object_ptr<Ui::FlatLabel>(this, tr::lng_contacts_loading(),
                                            st::changePhoneDescription),
                  std::move(padding)));
    Ui::ResizeFitChild(this, _loading.get(), st::settingsBlockedHeightMin);
  }
...
QPointer<Ui::RpWidget> Blocked::createPinnedToTop(not_null<QWidget *> parent) {
  ...
  AddButtonWithIcon(content, tr::lng_blocked_list_add(),
                    st::settingsButtonActive, {&st::menuIconBlockSettings})
      ->addClickHandler(
...

  {
    ...
    auto subtitleText =
        _countBlocked.value() | rpl::map([=](int count) {
          return tr::lng_blocked_list_subtitle(tr::now, lt_count, count);
        });
    Ui::AddSubsectionTitle(subtitle->entity(), rpl::duplicate(subtitleText),
                           st::settingsBlockedListSubtitleAddPadding);
    subtitle->toggleOn(
        rpl::merge(_emptinessChanges.events() | rpl::map(!rpl::mappers::_1),
...
void Blocked::setupContent() {
...

  {
    ...
    state->controller->rowsCountChanges() | rpl::start_with_next(
                                                [=](int total) {
                                                  _countBlocked = total;
                                                  checkTotal(total);
                                                },
...

  {
    ...
    _showFinished.events() | rpl::start_with_next(
    ...
    content->add(object_ptr<Ui::CenterWrap<>>(
                     content, object_ptr<Ui::FlatLabel>(
                                  content, tr::lng_blocked_list_empty_title(),
                                  st::changePhoneTitle)),
    ...
    content->add(
        object_ptr<Ui::CenterWrap<>>(
            content, object_ptr<Ui::FlatLabel>(
                         content, tr::lng_blocked_list_empty_description(),
                         st::changePhoneDescription)),
  ...
  widthValue() |
      rpl::start_with_next([=](int width) { _container->resizeToWidth(width); },
                           _container->lifetime());

  rpl::combine(_container->heightValue(),
      ...
      rpl::start_with_next(
          [=](int height, bool empty) {
            ...
            const auto padding = st::defaultSubsectionTitlePadding +
                                 st::settingsBlockedListSubtitleAddPadding;
            const auto subtitle =
                st::defaultVerticalListSkip + padding.top() +
...
void Blocked::showFinished() { _showFinished.fire({}); }
