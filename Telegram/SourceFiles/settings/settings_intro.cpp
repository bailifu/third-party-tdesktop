...
object_ptr<Ui::RpWidget> CreateIntroSettings(
    QWidget *parent, not_null<Window::Controller *> window) {
  ...
  }
  {
    ...
    SetupSystemIntegrationContent(window->sessionController(), wrap.data());
    SetupWindowTitleContent(window->sessionController(), wrap.data());
    if (wrap->count() > 0) {
      Ui::AddDivider(result);
      Ui::AddSkip(result);
      result->add(object_ptr<Ui::OverrideMargins>(result, std::move(wrap)));
      Ui::AddSkip(result);
    }
  }
  Ui::AddDivider(result);
  ...
  AddButtonWithIcon(result, tr::lng_settings_faq(), st::settingsButtonNoIcon)
      ->addClickHandler([] { OpenFaq(nullptr); });

  return result;
}

TopBar::TopBar(QWidget *parent, const style::InfoTopBar &st)
    : RpWidget(parent), _st(st) {}

void TopBar::setTitle(rpl::producer<QString> &&title) {
  if (_title) {
    delete _title;
  }
  _title = Ui::CreateChild<Ui::FlatLabel>(this, std::move(title), _st.title);
...
Ui::RpWidget *TopBar::pushButton(base::unique_qptr<Ui::RpWidget> button) {
  ...
  weak->widthValue() |
      rpl::start_with_next([this] { updateControlsGeometry(width()); },
                           lifetime());
  return weak;
}
...
void TopBar::updateControlsGeometry(int newWidth) {
  ...
  if (_title) {
    _title->moveToLeft(_st.titlePosition.x(), _st.titlePosition.y(), newWidth);
  }
}
...
void TopBar::paintEvent(QPaintEvent *e) {
  ...
  QPainter(this).fillRect(
      e->rect().intersected({0, radius, width(), height() - radius}), _st.bg);
}
...
class IntroWidget : public Ui::RpWidget {
 public:
  IntroWidget(QWidget *parent, not_null<Window::Controller *> window);
...
IntroWidget::IntroWidget(QWidget *parent, not_null<Window::Controller *> window)
    : RpWidget(parent),
      _wrap(this),
  ...
  _scrollTopSkip.changes() |
      rpl::start_with_next([this] { updateControlsGeometry(); }, lifetime());

  createTopBar(window);
  showContent(window);
  _topShadow->toggleOn(topShadowToggledValue() |
                       rpl::filter([](bool shown) { return true; }));
}
...
void IntroWidget::updateControlsGeometry() {
  ...
  auto scrollGeometry =
      _wrap->rect().marginsRemoved(QMargins(0, _scrollTopSkip.current(), 0, 0));
  ...
  if (!_scroll->isHidden()) {
    ...
    _innerWrap->setVisibleTopBottom(scrollTop, scrollTop + _scroll->height());
  }
}
...
void IntroWidget::createTopBar(not_null<Window::Controller *> window) {
  ...
  auto close = _topBar->addButton(
      base::make_unique_q<Ui::IconButton>(_topBar, st::infoLayerTopBarClose));
  close->addClickHandler([=] { window->hideSettingsAndLayer(); });
...
void IntroWidget::setInnerWidget(object_ptr<Ui::RpWidget> content) {
  _innerWrap =
      _scroll->setOwnedWidget(object_ptr<Ui::PaddingWrap<Ui::RpWidget>>(
          this, std::move(content),
  ...
  rpl::combine(_scroll->scrollTopValue(), _scroll->heightValue(),
      ...
      rpl::start_with_next(
          [this](int top, int height, int desired) {
...
rpl::producer<bool> IntroWidget::topShadowToggledValue() const {
  ...
  return rpl::combine(_scroll->scrollTopValue(), _scrollTopSkip.value()) |
         rpl::map((_1 > 0) || (_2 > 0));
}
...
void IntroWidget::setInnerFocus() { setFocus(); }

rpl::producer<int> IntroWidget::desiredHeightValue() const {
  using namespace rpl::mappers;
  return rpl::combine(_topBar->heightValue(),
                      _innerWrap->entity()->desiredHeightValue(),
...
void IntroWidget::resizeEvent(QResizeEvent *e) { updateControlsGeometry(); }

void IntroWidget::keyPressEvent(QKeyEvent *e) {
  crl::on_main(this, [text = e->text()] { CodesFeedString(nullptr, text); });
...
int IntroWidget::scrollTillBottom(int forHeight) const {
  auto scrollHeight = forHeight - _scrollTopSkip.current() - _topBar->height();
...
void LayerWidget::setupHeightConsumers() {
  _content->scrollTillBottomChanges() |
      ...
      rpl::start_with_next([this] { resizeToWidth(width()); }, lifetime());
  _content->desiredHeightValue() | rpl::start_with_next(
                                       [this](int height) {
...
void LayerWidget::parentResized() {
  ...
  const auto newWidth = (parentWidth < MinimalSupportedWidth())
                            ? parentWidth
                            : qMin(parentWidth - 2 * st::infoMinimalLayerMargin,
                                   st::infoDesiredWidth);
...
int LayerWidget::resizeGetHeight(int newWidth) {
  ...
  auto newTop = std::clamp(windowHeight / 24, st::infoLayerTopMinimal,
  ...
  _content->updateGeometry(
      {contentLeft, contentTop, contentWidth, contentHeight}, additionalScroll);
...
void LayerWidget::doSetInnerFocus() { _content->setInnerFocus(); }
