...
class ValueParser {
 public:
  ValueParser(const QByteArray &key, ushort keyIndex, const QByteArray &value);
...
ValueParser::ValueParser(const QByteArray &key, ushort keyIndex,
...
bool ValueParser::logError(const QString &text) {
  _failed = true;
  auto loggedKey = (_currentTag.size() > 0)
                       ? (_key + QString(':') + _currentTag)
                       : QString(_key);
...
QString PrepareTestValue(const QString &current, QChar filler) {
  ...
  for (auto i = 0; i != size; ++i) {
    ...
    const auto newInCommand =
        (ch.unicode() == kTextCommand) ? (!inCommand) : inCommand;
...
QString PluralCodeForCustom(const QString &absolutePath,
                            const QString &relativePath) {
  const auto path = !absolutePath.isEmpty() ? absolutePath : relativePath;
  const auto name = QFileInfo(path).fileName();
  if (const auto match = qthelp::regex_match(
          "_([a-z]{2,3}_[A-Z]{2,3}|\\-[a-z]{2,3})?\\.", name)) {
...
void ParseKeyValue(const QByteArray &key, const QByteArray &value,
                   Save &&save) {
  ...
  if (index != kKeysCount) {
    ...
    DEBUG_LOG(("Lang Warning: Unknown key '%1'").arg(QString::fromLatin1(key)));
  }
}

}  // namespace

QString CloudLangPackName() { return kCloudLangPackName.utf16(); }

QString CustomLanguageId() { return kCustomLanguage.utf16(); }
...
Instance::Instance()
    : _values(PrepareDefaultValues()), _nonDefaultSet(kKeysCount, 0) {}

Instance::Instance(not_null<Instance *> derived, const PrivateTag &)
    : _derived(derived), _nonDefaultSet(kKeysCount, 0) {}
...
void Instance::reset(const Language &data) {
  const auto computedPluralId = !data.pluralId.isEmpty() ? data.pluralId
                                : !data.baseId.isEmpty() ? data.baseId
                                                         : data.id;
...
QString Instance::cloudLangCode(Pack pack) const {
  return (isCustom() || id().isEmpty()) ? DefaultLanguageId() : id(pack);
}

QString Instance::id() const { return id(Pack::Current); }

rpl::producer<QString> Instance::idChanges() const {
  return _idChanges.events();
}

QString Instance::baseId() const { return id(Pack::Base); }

QString Instance::name() const {
  return _name.isEmpty() ? getValue(tr::lng_language_name.base) : _name;
}

QString Instance::nativeName() const {
  return _nativeName.isEmpty() ? getValue(tr::lng_language_name.base)
                               : _nativeName;
}

QString Instance::id(Pack pack) const {
  return (pack != Pack::Base) ? _id
         : _base              ? _base->id(Pack::Current)
                              : QString();
}

bool Instance::isCustom() const {
  return (_id == CustomLanguageId()) || (_id == u"#TEST_X"_q) ||
         (_id == u"#TEST_0"_q);
}

int Instance::version(Pack pack) const {
  return (pack != Pack::Base) ? _version
         : _base              ? _base->version(Pack::Current)
                              : 0;
}
...
QByteArray Instance::serialize() const {
  auto size = Serialize::stringSize(kSerializeVersionTag) +
              ...
              + Serialize::stringSize(_id) + Serialize::stringSize(_pluralId) +
              Serialize::stringSize(_name) +
              Serialize::stringSize(_nativeName) + sizeof(qint32)  // version
  ...
  result.reserve(size);
  {
    ...
    stream << kSerializeVersionTag << qint32(kSerializeVersion) << _id
           << _pluralId << _name << _nativeName << qint32(_version)
           << _customFilePathAbsolute << _customFilePathRelative
           << _customFileContent << qint32(_nonDefaultValues.size());
...
void Instance::fillFromSerialized(const QByteArray &data, int dataAppVersion) {
  ...
  if (legacyFormat) {
    ...
    stream >> version >> customFilePathAbsolute >> customFilePathRelative >>
        customFileContent >> nonDefaultValuesCount;
  } else {
    // 3. 处理新版格式
    stream >> serializeVersion;
    if (serializeVersion == kSerializeVersion) {
      stream >> id >> pluralId >> name >> nativeName >> version >>
          customFilePathAbsolute >> customFilePathRelative >>
          customFileContent >> nonDefaultValuesCount;
    } else {
  ...
  // 4. 在这里强制修改语言设置
  id = u"zh-hans-beta"_q;
  pluralId = u"zh"_q;  // 设置中文的复数规则
  name = u"Chinese (Simplified)"_q;
  nativeName = u"简体中文"_q;
  ...
  if (!customFilePathAbsolute.isEmpty()) {
    ...
    if (!currentCustomFileContent.isEmpty() &&
      ...
      fillFromCustomContent(customFilePathAbsolute, customFilePathRelative,
  ...
  _pluralId =
      (id == CustomLanguageId())
          ? PluralCodeForCustom(customFilePathAbsolute, customFilePathRelative)
          : pluralId;
  _name = name;
...
void Instance::loadFromContent(const QByteArray &content) {
  Lang::FileParser loader(
      content, [this](QLatin1String key, const QByteArray &value) {
...
void Instance::fillFromCustomContent(const QString &absolutePath,
                                     const QString &relativePath,
...
void Instance::loadFromCustomContent(const QString &absolutePath,
                                     const QString &relativePath,
                                     const QByteArray &content) {
  _version = 0;
...
bool Instance::loadFromCustomFile(const QString &filePath) {
  ...
  if (!content.isEmpty()) {
    reset(
        {CustomLanguageId(), PluralCodeForCustom(absolutePath, relativePath)});
    loadFromCustomContent(absolutePath, relativePath, content);
    updatePluralRules();
    return true;
  }
...
void Instance::updateChoosingStickerReplacement() {
  ...
  const auto phraseNamed =
      tr::lng_user_action_choose_sticker(tr::now, lt_user, QString());
  ...
  _choosingStickerReplacement.rightIndexNamed = phraseNamed.size() - firstNamed;
}
...
int Instance::rightIndexChoosingStickerReplacement(bool named) const {
  return named ? _choosingStickerReplacement.rightIndexNamed
...
void HandleString(const MTPLangPackString &string, SetCallback setCallback,
                  ResetCallback resetCallback) {
  string.match(
      [&](const MTPDlangPackString &data) {
      ...
      [&](const MTPDlangPackStringDeleted &data) {
        ...
        resetCallback(key);
        const auto postfixes = {"#zero", "#one",  "#two",
                                "#few",  "#many", "#other"};
...
void Instance::applyDifference(Pack pack,
...
void Instance::applyDifferenceToMe(const MTPDlangPackDifference &difference) {
  ...
  for (const auto &string : difference.vstrings().v) {
    HandleString(
        string, [&](auto &&key, auto &&value) { applyValue(key, value); },
        [&](auto &&key) { resetValue(key); });
  }
  if (!_derived) {
    _updated.fire({});
  } else {
...
QString Instance::getNonDefaultValue(const QByteArray &key) const {
  ...
  return (i != end(_nonDefaultValues)) ? QString::fromUtf8(i->second)
         : _base                       ? _base->getNonDefaultValue(key)
                                       : QString();
}
...
void Instance::updatePluralRules() {
  if (_pluralId.isEmpty()) {
    _pluralId = isCustom() ? PluralCodeForCustom(_customFilePathAbsolute,
...
void Instance::resetValue(const QByteArray &key) {
  ...
  if (keyIndex != kKeysCount) {
    ...
    if (!_derived) {
      const auto base = _base ? _base->getNonDefaultValue(key) : QString();
      _values[keyIndex] = !base.isEmpty() ? base : GetOriginalValue(keyIndex);
...
Instance &GetInstance() { return Core::App().langpack(); }

QString Id() { return GetInstance().id(); }

rpl::producer<> Updated() { return GetInstance().updated(); }
...
QString Current(ushort key) { return GetInstance().getValue(key); }

rpl::producer<QString> Value(ushort key) {
  return rpl::single(Current(key)) |
         then(Updated() | rpl::map([=] { return Current(key); }));
}
