...
class Row final : public PeerListRow {
 public:
  ...
  void elementAddRipple(int element, QPoint point,
                        Fn<void()> updateCallback) override;
  void elementsStopLastRipple() override;
  void elementsPaint(Painter &p, int outerWidth, bool selected,
...
void RenameBox(not_null<Ui::GenericBox *> box) {
  ...
  box->addRow(object_ptr<Ui::FlatLabel>(box, tr::lng_settings_device_name(),
                                        st::defaultSubsectionTitle),
              st::boxRowPadding + style::margins(0, skip, 0, 0));
  const auto name = box->addRow(
      object_ptr<Ui::InputField>(box, st::settingsDeviceName,
                                 rpl::single(Platform::DeviceModelPretty()),
                                 Core::App().settings().customDeviceModel()),
      st::boxRowPadding -
          style::margins(st::settingsDeviceName.textMargins.left(), 0,
                         st::settingsDeviceName.textMargins.right(), 0));
  name->setMaxLength(kMaxDeviceModelLength);
  box->setFocusCallback([=] { name->setFocusFast(); });
  const auto submit = [=] {
    const auto result = base::CleanAndSimplify(name->getLastText());
    box->closeBox();
...
[[nodiscard]] QString LocationAndDate(const EntryData &entry) {
  return (entry.location.isEmpty() ? entry.ip : entry.location) +
         (entry.hash ? (QString::fromUtf8(" \xE2\x80\xA2 ") + entry.active)
                     : QString());
}
...
[[nodiscard]] Type TypeFromEntry(const EntryData &entry) {
  ...
  const auto kAndroid = std::array{5, 6, 24, 1026, 1083, 2458, 2521, 21724};
  ...
  const auto detectBrowser = [&]() -> std::optional<Type> {
    if (device.contains("edg/") || device.contains("edgios/") ||
  ...
  const auto detectDesktop = [&]() -> std::optional<Type> {
    if (platform.contains("windows") || system.contains("windows")) {
      ...
      return Type::Mac;
    } else if (platform.contains("ubuntu") || system.contains("ubuntu") ||
               platform.contains("unity") || system.contains("unity")) {
      return Type::Ubuntu;
...
[[nodiscard]] const style::icon &IconForType(Type type) {
  switch (type) {
    case Type::Windows:
      return st::sessionIconWindows;
    case Type::Mac:
      return st::sessionIconMac;
    case Type::Ubuntu:
      return st::sessionIconUbuntu;
    case Type::Linux:
      return st::sessionIconLinux;
    case Type::iPhone:
      return st::sessionIconiPhone;
    case Type::iPad:
      return st::sessionIconiPad;
    case Type::Android:
      return st::sessionIconAndroid;
    case Type::Web:
      return st::sessionIconWeb;
    case Type::Chrome:
      return st::sessionIconChrome;
    case Type::Edge:
      return st::sessionIconEdge;
    case Type::Firefox:
      return st::sessionIconFirefox;
    case Type::Safari:
      return st::sessionIconSafari;
    case Type::Other:
      return st::sessionIconOther;
  }
...
[[nodiscard]] const style::icon *IconBigForType(Type type) {
  switch (type) {
    case Type::Web:
      return &st::sessionBigIconWeb;
    case Type::Other:
      return &st::sessionBigIconOther;
  }
  return nullptr;
}
...
[[nodiscard]] std::unique_ptr<Lottie::Icon> LottieForType(Type type) {
  ...
  const auto path = [&] {
    switch (type) {
      case Type::Windows:
        return "device_desktop_win";
      case Type::Mac:
        return "device_desktop_mac";
      case Type::Ubuntu:
        return "device_linux_ubuntu";
      case Type::Linux:
        return "device_linux";
      case Type::iPhone:
        return "device_phone_ios";
      case Type::iPad:
        return "device_tablet_ios";
      case Type::Android:
        return "device_phone_android";
      case Type::Chrome:
        return "device_web_chrome";
      case Type::Edge:
        return "device_web_edge";
      case Type::Firefox:
        return "device_web_firefox";
      case Type::Safari:
        return "device_web_safari";
    }
...
[[nodiscard]] not_null<Ui::RpWidget *> GenerateUserpicBig(
    not_null<Ui::RpWidget *> parent, rpl::producer<> shown, Type type) {
  ...
  state->background = QImage(full, full, QImage::Format_ARGB32_Premultiplied);
  ...
  if ((state->lottie = LottieForType(type))) {
    std::move(shown) |
        rpl::start_with_next(
            [=] {
              state->lottie->animate([=] { result->update(); }, 0,
      ...
      rpl::start_with_next(
          [=] {
            ...
            if (state->lottie) {
              ...
              q.end();
              style::colorizeImage(state->lottieFrame,
...
void SessionInfoBox(not_null<Ui::GenericBox *> box, const EntryData &data,
                    Fn<void(uint64)> terminate) {
  box->setWidth(st::boxWideWidth);

  const auto shown = box->lifetime().make_state<rpl::event_stream<>>();
  box->setShowFinishedCallback([=] { shown->fire({}); });

  const auto userpicWrap = box->addRow(
      object_ptr<Ui::FixedHeightWidget>(box, st::sessionBigUserpicSize),
      st::sessionBigCoverPadding);
  const auto big =
      GenerateUserpicBig(userpicWrap, shown->events(), TypeFromEntry(data));
  userpicWrap->sizeValue() |
      rpl::start_with_next(
          [=](QSize size) { big->move((size.width() - big->width()) / 2, 0); },
          userpicWrap->lifetime());

  const auto nameWrap = box->addRow(
      object_ptr<Ui::FixedHeightWidget>(box, st::sessionBigName.maxHeight));
  const auto name = Ui::CreateChild<Ui::FlatLabel>(
      nameWrap, rpl::single(data.name), st::sessionBigName);
  nameWrap->widthValue() | rpl::start_with_next(
                               [=](int width) {
  ...
  const auto dateWrap =
      box->addRow(object_ptr<Ui::FixedHeightWidget>(
                      box, st::sessionDateLabel.style.font->height),
                  style::margins(0, 0, 0, st::sessionDateSkip));
  const auto date = Ui::CreateChild<Ui::FlatLabel>(
      dateWrap,
      rpl::single(langDateTimeFull(base::unixtime::parse(data.activeTime))),
      st::sessionDateLabel);
  rpl::combine(dateWrap->widthValue(), date->widthValue()) |
      rpl::start_with_next(
          [=](int outer, int inner) { date->move((outer - inner) / 2, 0); },
          date->lifetime());
  ...
  AddSessionInfoRow(container, tr::lng_sessions_application(), data.info,
                    st::menuIconDevices);
  AddSessionInfoRow(container, tr::lng_sessions_system(), data.system,
                    st::menuIconInfo);
  AddSessionInfoRow(container, tr::lng_sessions_ip(), data.ip,
                    st::menuIconIpAddress);
  AddSessionInfoRow(container, tr::lng_sessions_location(), data.location,
                    st::menuIconAddress);
...
EntryData Row::data() const { return _data; }

QString Row::generateName() { return _data.name; }

QString Row::generateShortName() { return generateName(); }
...
int Row::elementsCount() const { return 2; }

QRect Row::elementGeometry(int element, int outerWidth) const {
  switch (element) {
    case 1: {
      return QRect(st::sessionListItem.namePosition.x(), st::sessionLocationTop,
                   outerWidth, st::normalFont->height);
    } break;
    case 2: {
      const auto size =
          QSize(st::sessionTerminate.width, st::sessionTerminate.height);
...
bool Row::elementDisabled(int element) const { return !id() || (element == 1); }

bool Row::elementOnlySelect(int element) const { return false; }

void Row::elementAddRipple(int element, QPoint point,
                           Fn<void()> updateCallback) {}

void Row::elementsStopLastRipple() {}

void Row::elementsPaint(Painter &p, int outerWidth, bool selected,
                        int selectedElement) {
  if (id()) {
    const auto geometry = elementGeometry(2, outerWidth);
    const auto position =
        geometry.topLeft() + st::sessionTerminate.iconPosition;
    const auto &icon = (selectedElement == 2) ? st::sessionTerminate.iconOver
  ...
  _location.drawLeftElided(p, locationLeft, st::sessionLocationTop, available,
                           outerWidth);
}

class SessionsContent : public Ui::RpWidget {
 public:
  SessionsContent(QWidget *, not_null<Window::SessionController *> controller);
...
class SessionsContent::ListController final : public PeerListController,
                                              public RowDelegate,
 ...
 public:
  ...
  [[nodiscard]] static std::unique_ptr<ListController> Add(
      ...
      not_null<Main::Session *> session, style::margins margins = {});
...
void SessionsContent::setupContent() {
  ...
  _inner->heightValue() | rpl::distinct_until_changed() |
      rpl::start_with_next([=](int height) { resize(width(), height); },
                           _inner->lifetime());

  _inner->showRequests() |
      rpl::start_with_next(
          [=](const EntryData &data) {
            _controller->show(Box(SessionInfoBox, data,
  ...
  _authorizations->listValue() |
      rpl::start_with_next(
          [=](const Api::Authorizations::List &list) { parse(list); },
          lifetime());
...
void SessionsContent::paintEvent(QPaintEvent *e) {
  ...
  if (_loading.current()) {
    ...
    p.drawText(QRect(0, 0, width(), st::noContactsHeight),
               tr::lng_contacts_loading(tr::now), style::al_center);
  }
}

void SessionsContent::shortPollSessions() {
  const auto left =
      kShortPollTimeout - (crl::now() - _authorizations->lastReceivedTime());
  if (left > 0) {
...
void SessionsContent::terminateOne(uint64 hash) {
  ...
  auto callback = [=] {
    ...
    _authorizations->requestTerminate(std::move(done), std::move(fail), hash);
  };
...
SessionsContent::Inner::Inner(QWidget *parent,
                              ...
                              rpl::producer<int> ttlDays)
    : RpWidget(parent), _controller(controller), _ttlDays(std::move(ttlDays)) {
  setupContent();
}
...
void SessionsContent::Inner::setupContent() {
  ...
  const auto header = AddSubsectionTitle(content, tr::lng_sessions_header());
  const auto rename = Ui::CreateChild<Ui::LinkButton>(
      content, tr::lng_settings_rename_device(tr::now), st::defaultLinkButton);
  rpl::combine(content->sizeValue(), header->positionValue()) |
      rpl::start_with_next(
          [=](QSize outer, QPoint position) {
          ...
          rename->lifetime());
  rename->setClickedCallback([=] { _controller->show(Box(RenameBox)); });
  ...
  AddButtonWithLabel(ttlInner, tr::lng_settings_terminate_if(),
                     _ttlDays.value() | rpl::map(SelfDestructionBox::DaysLabel),
                     st::settingsButtonNoIcon)
      ->addClickHandler([=] {
        _controller->show(Box<SelfDestructionBox>(
            &_controller->session(), SelfDestructionBox::Type::Sessions,
            _ttlDays.value()));
      });
...
void SessionsContent::ListController::rowClicked(not_null<PeerListRow *> row) {
  _showRequests.fire_copy(static_cast<Row *>(row.get())->data());
}

void SessionsContent::ListController::rowElementClicked(
    not_null<PeerListRow *> row, int element) {
  if (element == 2) {
...
void SessionsContent::ListController::showData(
  ...
  for (const auto &entry : items) {
    ...
    if (const auto row = delegate()->peerListFindRow(id)) {
      ...
      delegate()->peerListAppendRow(std::make_unique<Row>(this, entry));
    }
  }
...
rpl::producer<int> SessionsContent::ListController::itemsCount() const {
  return _itemsCount.events_starting_with(delegate()->peerListFullRowsCount());
}

rpl::producer<uint64> SessionsContent::ListController::terminateRequests()
    const {
...
auto SessionsContent::ListController::Add(
    not_null<Ui::VerticalLayout *> container, not_null<Main::Session *> session,
    style::margins margins) -> std::unique_ptr<ListController> {
  ...
  const auto content = container->add(
      object_ptr<PeerListContent>(container, controller.get()), margins);
...
void Sessions::setupContent(not_null<Window::SessionController *> controller) {
  ...
  const auto content =
      container->add(object_ptr<SessionsContent>(container, controller));
  content->setupContent();

  Ui::ResizeFitChild(this, container);
}

void AddSessionInfoRow(not_null<Ui::VerticalLayout *> container,
                       rpl::producer<QString> label, const QString &value,
  ...
  const auto text = container->add(
      object_ptr<Ui::FlatLabel>(container, rpl::single(value), st::boxLabel),
      st::boxRowPadding + st::sessionValuePadding);
  const auto left = st::sessionValuePadding.left();
  container->add(
      object_ptr<Ui::FlatLabel>(container, std::move(label),
                                st::sessionValueLabel),
      (st::boxRowPadding + style::margins{left, 0, 0, st::sessionValueSkip}));
