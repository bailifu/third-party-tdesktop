...
const char kOptionAutoScrollInactiveChat[] = "auto-scroll-inactive-chat";
...
MainWindow::MainWindow(not_null<Window::Controller *> controller)
  ...
  Window::Theme::Background()->updates() |
      rpl::start_with_next(
          [=](const BackgroundUpdate &data) { themeUpdated(data); },
          lifetime());

  Core::App().passcodeLockChanges() |
      rpl::start_with_next([=] { updateGlobalMenu(); }, lifetime());

  Ui::Emoji::Updated() |
      rpl::start_with_next([=] { Ui::ForceFullRepaint(this); }, lifetime());
...
void MainWindow::finishFirstShow() {
  ...
  windowActiveValue() | rpl::skip(1) | rpl::filter(!rpl::mappers::_1) |
      rpl::start_with_next([=] { Ui::Tooltip::Hide(); }, lifetime());
...
void MainWindow::setupIntro(Intro::EnterPoint point, QPixmap oldContentCache) {
  auto animated = (_main || _passcodeLock);

  destroyLayer();
  auto created =
      object_ptr<Intro::Widget>(bodyWidget(), &controller(), &account(), point);
  created->showSettingsRequested() |
      rpl::start_with_next([=] { showSettings(); }, created->lifetime());
...
void MainWindow::setupMain(MsgId singlePeerShowAtMsgId,
  ...
  const auto animated =
      _intro || (_passcodeLock && !Core::App().passcodeLocked());
  ...
  _main->controller()->showByInitialId(Window::SectionShow::Way::ClearStack,
...
void MainWindow::showSettings() {
  ...
  if (const auto session = sessionController()) {
    ...
    showSpecialLayer(Box<Settings::LayerWidget>(&controller()),
                     anim::type::normal);
  }
}

void MainWindow::showSpecialLayer(object_ptr<Ui::LayerWidget> layer,
                                  anim::type animated) {
  if (_passcodeLock) {
    return;
  }
...
void MainWindow::ensureLayerCreated() {
  ...
  _layer = base::make_unique_q<Ui::LayerStackWidget>(
      bodyWidget(), crl::guard(this, [=] { return controller().uiShow(); }));

  _layer->hideFinishEvents() | rpl::filter([=] {
    return _layer != nullptr;  // Last hide finish is sent from destructor.
  }) | rpl::start_with_next([=] { destroyLayer(); }, _layer->lifetime());
...
void MainWindow::destroyLayer() {
  ...
  InvokeQueued(this, [=] { checkActivation(); });
}
...
MainWidget *MainWindow::sessionContent() const { return _main.data(); }

void MainWindow::showOrHideBoxOrLayer(
    std::variant<v::null_t, object_ptr<Ui::BoxContent>,
                 std::unique_ptr<Ui::LayerWidget>> &&layer,
    Ui::LayerOptions options, anim::type animated) {
  ...
  if (auto layerWidget = std::get_if<UniqueLayer>(&layer)) {
    ...
    if (_layer) {
      ...
      if ((animated == anim::type::instant) && _layer &&
          !_layer->layerShown()) {
        destroyLayer();
      }
    }
...
bool MainWindow::ui_isLayerShown() const { return _layer != nullptr; }

bool MainWindow::showMediaPreview(Data::FileOrigin origin,
                                  not_null<DocumentData *> document) {
  const auto media = document->activeMediaView();
  const auto preview = Data::VideoPreviewState(media.get());
  if (!document->sticker() && (!document->isAnimation() || !preview.loaded())) {
    return false;
  }
  if (!_mediaPreview) {
...
void MainWindow::themeUpdated(const Window::Theme::BackgroundUpdate &data) {
  ...
  if (data.type == Type::TestingTheme) {
    if (!_testingThemeWarning) {
      ...
      _testingThemeWarning->setHiddenCallback(
          [this] { _testingThemeWarning.destroyDelayed(); });
    }
    crl::on_main(this, [=] {
      if (_testingThemeWarning) {
  ...
  } else if (data.type == Type::RevertingTheme ||
             data.type == Type::ApplyingTheme) {
    if (_testingThemeWarning) {
...
bool MainWindow::markingAsRead() const {
  return _main && !_main->isHidden() && !_main->animatingShow() && !_layer &&
         !isHidden() && !isMinimized() && windowHandle()->isExposed() &&
...
bool MainWindow::eventFilter(QObject *object, QEvent *e) {
  switch (e->type()) {
    case QEvent::KeyPress: {
      if (Logs::DebugEnabled() && object == windowHandle()) {
    ...
    case QEvent::ApplicationActivate: {
      if (object == QCoreApplication::instance()) {
        InvokeQueued(this, [=] { handleActiveChanged(isActiveWindow()); });
      }
    } break;
...
void MainWindow::updateControlsGeometry() {
  ...
  if (_main) {
    _main->setGeometry(
        {body.x() + mainLeft, body.y(), mainWidth, body.height()});
  }
