...
struct ReplaceTag<StringWithReacted> {
  static StringWithReacted Call(StringWithReacted &&original, ushort tag,
                                const StringWithReacted &replacement);
};

StringWithReacted ReplaceTag<StringWithReacted>::Call(
    StringWithReacted &&original, ushort tag,
    const StringWithReacted &replacement) {
...
class Action final : public Menu::ItemBase {
 public:
  Action(not_null<PopupMenu *> parentMenu,
         rpl::producer<WhoReadContent> content, CustomEmojiFactory factory,
...
class WhenAction final : public Menu::ItemBase {
 public:
  WhenAction(not_null<PopupMenu *> parentMenu,
             rpl::producer<WhoReadContent> content, Fn<void()> showOrPremium);
...
[[nodiscard]] QString FormatReactedString(int reacted, int seen) {
  ...
  return tr::lng_context_seen_reacted(tr::now, lt_count_short, reacted,
                                      projection)
      .text;
}

Action::Action(not_null<PopupMenu *> parentMenu,
    ...
    : ItemBase(parentMenu->menu(), parentMenu->menu()->st()),
      ...
      _userpics(std::make_unique<GroupCallUserpics>(
          st::defaultWhoRead.userpics, rpl::never<bool>(), [=] { update(); })),
  ...
  _userpics->widthValue() | rpl::start_with_next(
                                [=](int width) {
...
void Action::updateUserpicsFromContent() {
  ...
  if (!_content.participants.empty()) {
    const auto count = std::min(int(_content.participants.size()),
...
void Action::populateSubmenu() {
  ...
  const auto submenu = _parentMenu->ensureSubmenu(action(), st::whoReadMenu);
...
void Action::paint(Painter &p) {
  ...
  if (_custom) {
    ...
    const auto &icon = (_content.fullReactionsCount)
                           ? (!enabled   ? st::whoReadReactionsDisabled
                              : selected ? st::whoReadReactionsOver
                                         : st::whoReadReactions)
                       : (_content.type == WhoReadType::Seen)
                           ? (!enabled   ? st::whoReadChecksDisabled
                              : selected ? st::whoReadChecksOver
                                         : st::whoReadChecks)
                           : (!enabled   ? st::whoReadPlayedDisabled
                              : selected ? st::whoReadPlayedOver
                                         : st::whoReadPlayed);
    icon.paint(p, st::defaultWhoRead.iconPosition, width());
  }
  p.setPen(!enabled   ? _st.itemFgDisabled
           : selected ? _st.itemFgOver
                      : _st.itemFg);
  _text.drawLeftElided(p, st::defaultWhoRead.itemPadding.left(),
                       st::defaultWhoRead.itemPadding.top(), _textWidth,
                       width());
  if (_appeared) {
    _userpics->paint(p, width() - st::defaultWhoRead.itemPadding.right(),
...
void Action::refreshText() {
  ...
  const auto onlySeenCount = ranges::count(
      _content.participants, QString(), &WhoReadParticipant::customEntityData);
  ...
  _text.setMarkedText(
      ...
      {((_content.state == WhoReadState::Unknown)
        ...
        : (usersCount == 1) ? _content.participants.front().name
        : (_content.fullReactionsCount > 0 &&
           _content.fullReactionsCount <= _content.fullReadCount)
            ? FormatReactedString(_content.fullReactionsCount,
        ...
        : (_content.type == WhoReadType::Reacted ||
           ...
           (count > 0 && onlySeenCount == 0))
            ? (count ? tr::lng_context_seen_reacted(tr::now, lt_count_short,
                                                    count)
                     : tr::lng_context_seen_reacted_none(tr::now))
        : (_content.type == WhoReadType::Watched)
            ? (count ? tr::lng_context_seen_watched(tr::now, lt_count, count)
                     : tr::lng_context_seen_watched_none(tr::now))
        : (_content.type == WhoReadType::Listened)
            ? (count ? tr::lng_context_seen_listened(tr::now, lt_count, count)
                     : tr::lng_context_seen_listened_none(tr::now))
            : (count ? tr::lng_context_seen_text(tr::now, lt_count, count)
...
void Action::refreshDimensions() {
  ...
  const auto goodWidth =
      padding.left() + textWidth +
      (_userpicsWidth ? (_st.itemStyle.font->spacew + _userpicsWidth) : 0) +
      padding.right();

  const auto w =
      std::clamp(goodWidth, _st.widthMin, std::max(minWidth(), _st.widthMin));
...
not_null<QAction *> Action::action() const { return _dummyAction; }
...
int Action::contentHeight() const { return _height; }
...
WhenAction::WhenAction(not_null<PopupMenu *> parentMenu,
                       rpl::producer<WhoReadContent> content,
    ...
    : ItemBase(parentMenu->menu(), parentMenu->menu()->st()),
      ...
      _st(parentMenu->menu()->st()),
      _height(st::whenReadPadding.top() + st::whenReadStyle.font->height +
  ...
  std::move(content) | rpl::start_with_next(
                           [=](WhoReadContent &&content) {
                             _content = content;
                             refreshText();
                             refreshDimensions();
...
void WhenAction::resolveMinWidth() {
  ...
  const auto added =
      st::whenReadShowPadding.left() + st::whenReadShowPadding.right();
  ...
  const auto maxTextWidth =
      ...
      std::max({
          ...
          (width(tr::lng_context_read_hidden(tr::now)) + st::whenReadSkip +
          ...
          width(tr::lng_mediaview_date_time(
              tr::now, lt_date,
              tr::lng_month_day(tr::now, lt_month,
                                Lang::MonthDay(sampleDate.month())(tr::now),
                                lt_day, QString::number(sampleDate.day())),
              lt_time, sampleTime)),
      });

  const auto maxWidth =
      st::whenReadPadding.left() + maxTextWidth + st::whenReadPadding.right();
  setMinWidth(maxWidth);
}
...
void WhenAction::paint(Painter &p) {
  ...
  const auto &icon = (_content.type == WhoReadType::Edited)
                     ...
                     : loading  ? st::whoReadChecksDisabled
                     : selected ? st::whoReadChecksOver
                                : st::whoReadChecks;
  icon.paint(p, st::whenReadIconPosition, width());
  p.setPen(loading ? _st.itemFgDisabled : _st.itemFg);
  _text.drawLeftElided(p, st::whenReadPadding.left(), st::whenReadPadding.top(),
                       _textWidth, width());
  if (!_show.isEmpty()) {
    ...
    _show.drawLeftElided(p, inner.x(), inner.y(), inner.width(), width());
  }
}
...
void WhenAction::refreshText() {
  _text.setMarkedText(
      ...
      {((_content.state == WhoReadState::Unknown)
            ? tr::lng_context_seen_loading(tr::now)
        : _content.participants.empty() ? tr::lng_context_read_hidden(tr::now)
                                        : _content.participants.front().date)},
      MenuTextOptions);
  if (_content.state == WhoReadState::MyHidden) {
    _show.setMarkedText(st::whenReadStyle, {tr::lng_context_read_show(tr::now)},
                        MenuTextOptions);
  } else {
    _show = Text::String();
  }
...
void WhenAction::refreshDimensions() {
  ...
  const auto goodWidth =
      padding.left() + textWidth +
      (showWidth ? (st::whenReadSkip + st::whenReadShowPadding.left() +
                    showWidth + st::whenReadShowPadding.right())
                 : 0) +
      padding.right();

  const auto w =
      std::clamp(goodWidth, _st.widthMin, std::max(width(), _st.widthMin));
  _textWidth = std::min(w - (goodWidth - textWidth), textWidth);
  if (showWidth) {
    _showRect =
        QRect(padding.left() + _textWidth + st::whenReadSkip,
              padding.top() - st::whenReadShowPadding.top(),
              (st::whenReadShowPadding.left() + showWidth +
               st::whenReadShowPadding.right()),
              (st::whenReadShowPadding.top() + st::whenReadStyle.font->height +
...
not_null<QAction *> WhenAction::action() const { return _dummyAction; }
...
int WhenAction::contentHeight() const { return _height; }

}  // namespace

WhoReactedEntryAction::WhoReactedEntryAction(
    not_null<RpWidget *> parent, CustomEmojiFactory customEmojiFactory,
    const style::Menu &st, Data &&data)
    : ItemBase(parent, st),
  ...
  paintRequest() |
      rpl::start_with_next([=] { paint(Painter(this)); }, lifetime());
...
bool WhoReactedEntryAction::isEnabled() const { return true; }

int WhoReactedEntryAction::contentHeight() const { return _height; }
...
void WhoReactedEntryAction::setData(Data &&data) {
  ...
  if (data.date.isEmpty()) {
    ...
    _date.setMarkedText(st::whoReadDateStyle, {data.date}, MenuTextOptions);
  }
  _type = data.type;
  ...
  const auto textWidth =
      std::max(_text.maxWidth(), st::whoReadDateSkip + _date.maxWidth());
  const auto &padding = _st.itemPadding;
  const auto rightSkip =
      padding.right() + (_custom ? (size + padding.right()) : 0);
  const auto goodWidth = st::defaultWhoRead.nameLeft + textWidth + rightSkip;
...
void WhoReactedEntryAction::paint(Painter &&p) {
  ...
  if (preloader) {
    ...
    st::menuIconReactions.paintInCenter(
        p, QRect(photoLeft, photoTop, photoSize, photoSize));
  }

  const auto withDate = !_date.isEmpty();
  const auto textTop = withDate ? st::whoReadNameWithDateTop
  ...
  if (_type == WhoReactedType::Preloader) {
    ...
    p.drawRoundedRect(st::defaultWhoRead.nameLeft,
                      textTop + (_st.itemStyle.font->height - height) / 2,
                      _textWidth, height, height / 2., height / 2.);
  } else {
    p.setPen(selected  ? _st.itemFgOver
             : enabled ? _st.itemFg
                       : _st.itemFgDisabled);
    _text.drawLeftElided(p, st::defaultWhoRead.nameLeft, textTop, _textWidth,
                         width());
  }
  if (_type == WhoReactedType::RefRecipient ||
      _type == WhoReactedType::RefRecipientNow) {
    p.setPen(selected ? _st.itemFgShortcutOver : _st.itemFgShortcut);
    _date.drawLeftElided(p, st::defaultWhoRead.nameLeft, st::whoReadDateTop,
                         _textWidth, width());
  } else if (withDate) {
    const auto iconPosition =
        QPoint(st::defaultWhoRead.nameLeft, st::whoReadDateTop) +
        st::whoReadDateChecksPosition;
    const auto icon = [&] {
      switch (_type) {
        case WhoReactedType::Viewed:
          return &(selected ? st::whoReadDateChecksOver
                            : st::whoReadDateChecks);
        case WhoReactedType::Reacted:
          return &(selected ? st::whoLikedDateHeartOver
                            : st::whoLikedDateHeart);
        case WhoReactedType::Reposted:
          return &(selected ? st::whoRepostedDateHeartOver
                            : st::whoRepostedDateHeart);
        case WhoReactedType::Forwarded:
          return &(selected ? st::whoForwardedDateHeartOver
    ...
    p.setPen(selected ? _st.itemFgShortcutOver : _st.itemFgShortcut);
    _date.drawLeftElided(p, st::defaultWhoRead.nameLeft + st::whoReadDateSkip,
                         st::whoReadDateTop, _textWidth - st::whoReadDateSkip,
                         width());
  }
  if (_custom) {
    const auto ratio = style::DevicePixelRatio();
    ...
    _custom->paint(
        p,
        {
            .textColor = (selected ? _st.itemFgOver : _st.itemFg)->c,
            .now = crl::now(),
            .position = QPoint(width() - _st.itemPadding.right() - size + skip,
...
bool operator==(const WhoReadParticipant &a, const WhoReadParticipant &b) {
  return (a.id == b.id) && (a.name == b.name) && (a.date == b.date) &&
...
base::unique_qptr<Menu::ItemBase> WhoReactedContextAction(
    not_null<PopupMenu *> menu, rpl::producer<WhoReadContent> content,
    CustomEmojiFactory factory, Fn<void(WhoReadParticipant)> participantChosen,
    Fn<void()> showAllChosen) {
  return base::make_unique_q<Action>(
      menu, std::move(content), std::move(factory),
      std::move(participantChosen), std::move(showAllChosen));
}
...
void WhoReactedListMenu::clear() { _actions.clear(); }

void WhoReactedListMenu::populate(not_null<PopupMenu *> menu,
  ...
  const auto actionsCount =
      int(content.participants.size()) + (addShowAll ? 1 : 0);
  ...
  const auto append = [&](WhoReactedEntryData &&data) {
    if (index < _actions.size()) {
      ...
      auto item = base::make_unique_q<WhoReactedEntryAction>(
          menu->menu(), _customEmojiFactory, menu->menu()->st(),
          std::move(data));
  ...
  for (const auto &participant : content.participants) {
    ...
    append({
        ...
        .type = (participant.dateReacted ? WhoReactedType::Reacted
