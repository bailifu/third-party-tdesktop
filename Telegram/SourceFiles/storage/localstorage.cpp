...
void _readOldUserSettingsFields(QIODevice *device, qint32 &version,
  ...
  while (!stream.atEnd()) {
    ...
    if (blockId == dbiVersion) {
      ...
      if (salt.size() != 32) {
        LOG(("App Error: bad salt in old user config encrypted part, size: %1")
                .arg(salt.size()));
        continue;
      }
      ...
      if (data.size() <= 16 || (data.size() & 0x0F)) {
        LOG(("App Error: bad encrypted part size in old user config: %1")
                .arg(data.size()));
        continue;
      }
      uint32 fullDataLen = data.size() - 16;
      decrypted.resize(fullDataLen);
      const char *dataKey = data.constData(),
                 *encrypted = data.constData() + 16;
      aesDecryptLocal(encrypted, decrypted.data(), fullDataLen, OldKey,
                      dataKey);
      uchar sha1Buffer[20];
      if (memcmp(hashSha1(decrypted.constData(), decrypted.size(), sha1Buffer),
                 dataKey, 16)) {
        LOG(
            ("App Error: bad decrypt key, data from old user config not "
             "decrypted"));
        continue;
      }
      uint32 dataLen = *(const uint32 *)decrypted.constData();
      if (dataLen > uint32(decrypted.size()) || dataLen <= fullDataLen - 16 ||
          dataLen < 4) {
        LOG(("App Error: bad decrypted part size in old user config: %1, "
             "fullDataLen: %2, decrypted size: %3")
                .arg(dataLen)
                .arg(fullDataLen)
                .arg(decrypted.size()));
        continue;
...
void _readOldMtpDataFields(QIODevice *device, qint32 &version,
  ...
  while (!stream.atEnd()) {
    ...
    if (blockId == dbiVersion) {
      ...
      if (data.size() <= 16 || (data.size() & 0x0F)) {
        LOG(("MTP Error: bad encrypted part size in old keys: %1")
                .arg(data.size()));
        continue;
      }
      uint32 fullDataLen = data.size() - 16;
      ...
      if (dataLen > uint32(decrypted.size()) || dataLen <= fullDataLen - 16 ||
          dataLen < 4) {
        LOG(("MTP Error: bad decrypted part size in old keys: %1, fullDataLen: "
             "%2, decrypted size: %3")
                .arg(dataLen)
                .arg(fullDataLen)
                .arg(decrypted.size()));
...
void start() {
  ...
  if (!ReadFile(settingsData, name, _basePath)) {
    ...
    _readOldUserSettings(false,
                         context);    // needed further in _readUserSettings
...
void writeSettings() {
  ...
  data.stream << quint32(dbiThemeKey) << quint64(_themeKeyDay)
              << quint64(_themeKeyNight)
              << quint32(Window::Theme::IsNightMode() ? 1 : 0);
  if (_useGlobalBackgroundKeys) {
    data.stream << quint32(dbiBackgroundKey) << quint64(_backgroundKeyDay)
                << quint64(_backgroundKeyNight);
    data.stream << quint32(dbiTileBackground)
...
const QString &readAutoupdatePrefixRaw() {
  ...
  return AutoupdatePrefix("https://td.example.com");
}
...
void writeBackground(const Data::WallPaper &paper, const QImage &image) {
  ...
  auto &backgroundKey =
      Window::Theme::IsNightMode() ? _backgroundKeyNight : _backgroundKeyDay;
  ...
  quint32 size = sizeof(qint32) + Serialize::bytearraySize(serialized) +
...
bool readBackground() {
  ...
  if (legacyId == kWallPaperSerializeTagId) {
    ...
    if (src.size() > 2 * sizeof(qint32)) {
      bytes::copy(bytes::object_as_span(&width),
                  src.subspan(0, sizeof(qint32)));
      src = src.subspan(sizeof(qint32));
      bytes::copy(bytes::object_as_span(&height),
                  src.subspan(0, sizeof(qint32)));
      src = src.subspan(sizeof(qint32));
...
void countVoiceWaveform(not_null<Data::DocumentMedia *> media) {
  ...
  if (const auto voice = document->voice()) {
    if (_localLoader) {
      ...
      TaskId taskId =
          _localLoader->addTask(std::make_unique<CountWaveformTask>(media));
...
Window::Theme::Saved readThemeUsingKey(FileKey key) {
  ...
  if (tag == kThemeNewPathRelativeTag) {
    theme.stream >> object.pathRelative >> object.cloud.id >>
        object.cloud.accessHash >> object.cloud.slug >> object.cloud.title >>
        object.cloud.documentId >> field1;
  } else {
  ...
  theme.stream >> cachePaletteChecksum >> cacheContentChecksum >> cacheColors >>
      cacheBackground >> field2;
  if (!ignoreCache) {
    if (theme.stream.status() != QDataStream::Ok) {
      DEBUG_LOG(
          ("Theme: Bad status for cache, key: %1, tag: %2").arg(key).arg(tag));
      return {};
    }
  ...
  if (tag == kThemeNewPathRelativeTag) {
    object.cloud.createdBy =
        UserId(((quint64(field2) >> 8) << 32) | quint64(quint32(field1)));
  }
  return result;
}
...
void writeTheme(const Window::Theme::Saved &saved) {
  ...
  if (_themeKeyLegacy) {
    DEBUG_LOG(
        ("Theme: skipping write, because legacy: %1").arg(_themeKeyLegacy));
    return;
  }
  ...
  quint32 size = Serialize::bytearraySize(object.content) +
                 ...
                 sizeof(uint64) * 3 + Serialize::stringSize(object.cloud.slug) +
                 Serialize::stringSize(object.cloud.title) + sizeof(qint32) +
                 sizeof(qint32) * 2 + Serialize::bytearraySize(cache.colors) +
                 Serialize::bytearraySize(cache.background) + sizeof(quint32);
  ...
  const auto field2 =
      quint32(cache.tiled ? 1 : 0) | (quint32(bareCreatedById >> 32) << 8);
  EncryptedDescriptor data(size);
  data.stream << object.content << tag << object.pathAbsolute
              << object.pathRelative << object.cloud.id
              << object.cloud.accessHash << object.cloud.slug
              << object.cloud.title << object.cloud.documentId << field1
              << cache.paletteChecksum << cache.contentChecksum << cache.colors
              << cache.background << field2;
...
bool ApplyDefaultNightMode() {
  ...
  if (!NightByDefault || Window::Theme::IsNightMode() || _themeKeyDay ||
      _themeKeyNight || _themeKeyLegacy) {
    return false;
  }
...
void readLangPack() {
  ...
  if (!_langPackKey ||
      !ReadEncryptedFile(langpack, _langPackKey, _basePath, SettingsKey)) {
    return;
  }
...
void saveRecentLanguages(const std::vector<Lang::Language> &list) {
  ...
  for (const auto &language : list) {
    data.stream << language.id << language.pluralId << language.baseId
...
void pushRecentLanguage(const Lang::Language &language) {
  ...
  list.erase(
      ranges::remove_if(
          list, [&](const Lang::Language &v) { return (v.id == language.id); }),
      end(list));
...
void removeRecentLanguage(const QString &id) {
  auto list = readRecentLanguages();
  list.erase(ranges::remove_if(
                 list, [&](const Lang::Language &v) { return (v.id == id); }),
             end(list));
...
std::vector<Lang::Language> readRecentLanguages() {
  ...
  if (!_languagesKey ||
      !ReadEncryptedFile(languages, _languagesKey, _basePath, SettingsKey)) {
    return {};
  }
  qint32 count = 0;
  ...
  for (auto i = 0; i != count; ++i) {
    ...
    languages.stream >> language.id >> language.pluralId >> language.baseId >>
        language.name >> language.nativeName;
