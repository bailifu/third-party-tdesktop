...
struct BIODeleter {
  void operator()(BIO *value) { BIO_free(value); }
};
...
class HttpChecker : public Checker {
 ...
 private:
  ...
  std::optional<QString> parseOldResponse(const QByteArray &response) const;
  std::optional<QString> parseResponse(const QByteArray &response) const;
  QString validateLatestUrl(uint64 availableVersion, bool isAvailableAlpha,
                            QString url) const;
...
class HttpLoaderActor : public QObject {
 public:
  HttpLoaderActor(not_null<HttpLoader *> parent, not_null<QThread *> thread,
                  const QString &url);
...
class MtpChecker : public Checker {
 ...
 private:
  ...
  FileLocation validateLatestLocation(uint64 availableVersion,
...
QString UpdatesFolder() { return cWorkingDir() + u"tupdates"_q; }

void ClearAll() { base::Platform::DeleteDirectory(UpdatesFolder()); }
...
QString ExtractFilename(const QString &url) {
  ...
  if (const auto match = expression.match(url); match.hasMatch()) {
    return match.captured(1).replace(QRegularExpression(u"[^a-zA-Z0-9_\\-]"_q),
                                     QString());
  }
  return QString();
}
...
bool UnpackUpdate(const QString &filepath) {
...
#if defined Q_OS_WIN && !defined TDESKTOP_USE_PACKAGED  // use Lzma SDK for win
  const int32 hSigLen = 128, hShaLen = 20, hPropsLen = LZMA_PROPS_SIZE,
              hOriginalSizeLen = sizeof(int32),
              hSize =
                  hSigLen + hShaLen + hPropsLen + hOriginalSizeLen;  // header
#else   // Q_OS_WIN && !TDESKTOP_USE_PACKAGED
  const int32 hSigLen = 128, hShaLen = 20, hPropsLen = 0,
              hOriginalSizeLen = sizeof(int32),
              hSize = hSigLen + hShaLen + hOriginalSizeLen;  // header
  ...
  QString tempDirPath = cWorkingDir() + u"tupdates/temp"_q,
          readyFilePath = cWorkingDir() + u"tupdates/temp/ready"_q;
  ...
  bool goodSha1 = !memcmp(
      compressed.constData() + hSigLen,
      hashSha1(compressed.constData() + hSigLen + hShaLen,
               compressedLen + hPropsLen + hOriginalSizeLen, sha1Buffer),
      hShaLen);
  if (!goodSha1) {
  ...
  RSA *pbKey = [] {
    const auto bio =
        MakeBIO(const_cast<char *>(AppBetaVersion ? UpdatesPublicBetaKey
                                                  : UpdatesPublicKey),
                -1);
  ...
  if (RSA_verify(NID_sha1, (const uchar *)(compressed.constData() + hSigLen),
                 hShaLen, (const uchar *)(compressed.constData()), hSigLen,
                 pbKey) != 1) {  // verify signature
    RSA_free(pbKey);

    // try other public key, if we update from beta to stable or vice versa
    pbKey = [] {
      const auto bio =
          MakeBIO(const_cast<char *>(AppBetaVersion ? UpdatesPublicKey
  ...
  memcpy(&uncompressedLen,
         compressed.constData() + hSigLen + hShaLen + hPropsLen,
         hOriginalSizeLen);
  ...
  int uncompressRes = LzmaUncompress(
      (uchar *)uncompressed.data(), &resultLen,
      (const uchar *)(compressed.constData() + hSize), &srcLen,
      (const uchar *)(compressed.constData() + hSigLen + hShaLen),
      LZMA_PROPS_SIZE);
  if (uncompressRes != SZ_OK) {
    LOG(("Update Error: could not uncompress lzma, code: %1")
            .arg(uncompressRes));
    return false;
  }
#else   // Q_OS_WIN && !TDESKTOP_USE_PACKAGED
  ...
  if (ret != LZMA_OK) {
    ...
    LOG(("Error initializing the decoder: %1 (error code %2)")
            .arg(msg)
            .arg(ret));
    return false;
  }
  ...
  if (stream.avail_in) {
    LOG(("Error in decompression, %1 bytes left in _in of %2 whole.")
            .arg(stream.avail_in)
            .arg(compressedLen));
    return false;
  } else if (stream.avail_out) {
    LOG(("Error in decompression, %1 bytes free left in _out of %2 whole.")
  ...
  if (res != LZMA_OK && res != LZMA_STREAM_END) {
    ...
    switch (res) {
      ...
      case LZMA_FORMAT_ERROR:
        msg = "The input data is not in the .xz format";
        break;
      case LZMA_OPTIONS_ERROR:
  ...
  quint32 version;
  {
    ...
    if (stream.status() != QDataStream::Ok) {
      LOG(("Update Error: cant read version from downloaded stream, status: %1")
              .arg(stream.status()));
      return false;
    }
    ...
    if (version == 0x7FFFFFFF) {  // alpha version
      stream >> alphaVersion;
      if (stream.status() != QDataStream::Ok) {
        LOG(("Update Error: cant read alpha version from downloaded stream, "
             "status: %1")
                .arg(stream.status()));
        return false;
      }
      if (!cAlphaVersion() || alphaVersion <= cAlphaVersion()) {
        LOG(("Update Error: downloaded alpha version %1 is not greater, than "
             "mine %2")
                .arg(alphaVersion)
                .arg(cAlphaVersion()));
        return false;
      ...
      LOG(("Update Error: downloaded version %1 is not greater, than mine %2")
              .arg(version)
              .arg(AppVersion));
      return false;
    }
    ...
    stream >> filesCount;
    if (stream.status() != QDataStream::Ok) {
      LOG(("Update Error: cant read files count from downloaded stream, "
           "status: %1")
              .arg(stream.status()));
      return false;
    }
    ...
    for (uint32 i = 0; i < filesCount; ++i) {
...
#endif  // !Q_OS_WIN
      if (stream.status() != QDataStream::Ok) {
        LOG(("Update Error: cant read file from downloaded stream, status: %1")
                .arg(stream.status()));
        return false;
      }
      if (fileSize != quint32(fileInnerData.size())) {
        LOG(("Update Error: bad file size %1 not matching data size %2")
                .arg(fileSize)
                .arg(fileInnerData.size()));
        return false;
      }

      QFile f(tempDirPath + '/' + relativeName);
      if (!QDir().mkpath(QFileInfo(f).absolutePath())) {
        LOG(("Update Error: cant mkpath for file '%1'")
                .arg(tempDirPath + '/' + relativeName));
        return false;
      }
      if (!f.open(QIODevice::WriteOnly)) {
        LOG(("Update Error: cant open file '%1' for writing")
                .arg(tempDirPath + '/' + relativeName));
        return false;
      }
      ...
      if (writtenBytes != fileSize) {
        f.close();
        LOG(("Update Error: cant write file '%1', desiredSize: %2, write "
             "result: %3")
                .arg(tempDirPath + '/' + relativeName)
                .arg(fileSize)
                .arg(writtenBytes));
    ...
    const auto versionLen =
        VersionInt(versionString.size() * sizeof(VersionChar));
...
bool ParseCommonMap(const QByteArray &json, bool testing, Callback &&callback) {
  ...
  for (const auto &type : list) {
    ...
    const auto compare =
        isAvailableAlpha ? availableVersion : availableVersion * 1000;
    const auto bestCompare = bestIsAvailableAlpha ? bestAvailableVersion
...
rpl::producer<> Checker::failed() const { return _failed.events(); }

bool Checker::testing() const { return _testing; }

void Checker::done(std::shared_ptr<Loader> result) {
  _ready.fire(std::move(result));
}

void Checker::fail() { _failed.fire({}); }

rpl::lifetime &Checker::lifetime() { return _lifetime; }
...
void HttpChecker::start() {
  ...
  _reply->connect(_reply, &QNetworkReply::errorOccurred,
                  [=](auto e) { gotFailure(e); });
}
...
void HttpChecker::gotFailure(QNetworkReply::NetworkError e) {
  LOG(("Update Error: "
       "could not get current version %1")
          .arg(e));
...
std::optional<QString> HttpChecker::parseOldResponse(
  ...
  const auto old =
      QRegularExpression(u"^\\s*(\\d+)\\s*:\\s*([\\x21-\\x7f]+)\\s*$"_q)
          .match(string);
  if (!old.hasMatch()) {
    return std::nullopt;
  }
  ...
  return validateLatestUrl(
      availableVersion, isAvailableAlpha,
...
std::optional<QString> HttpChecker::parseResponse(
  ...
  const auto accumulate = [&](uint64 version, bool isAlpha,
    ...
    if (link == map.constEnd()) {
      LOG(("Update Error: Link not found for version %1.").arg(version));
      return false;
    } else if (!(*link).isString()) {
      LOG(("Update Error: Link is not a string for version %1.").arg(version));
      return false;
    }
  ...
  return validateLatestUrl(bestAvailableVersion, bestIsAvailableAlpha,
                           Local::readAutoupdatePrefix() + bestLink);
}

QString HttpChecker::validateLatestUrl(uint64 availableVersion,
                                       bool isAvailableAlpha,
                                       QString url) const {
  const auto myVersion =
      isAvailableAlpha ? cAlphaVersion() : uint64(AppVersion);
  const auto validVersion = (cAlphaVersion() || !isAvailableAlpha);
  if (!validVersion || availableVersion <= myVersion) {
    return QString();
  }
  const auto versionUrl =
      url.replace("{version}", QString::number(availableVersion));
  const auto finalUrl =
      isAvailableAlpha
          ? QString(versionUrl)
                .replace("{signature}",
...
HttpChecker::~HttpChecker() { clearSentRequest(); }
...
HttpLoader::~HttpLoader() {
  if (const auto thread = base::take(_thread)) {
    if (const auto actor = base::take(_actor)) {
      QObject::connect(thread.get(), &QThread::finished, actor,
                       &QObject::deleteLater);
    }
    thread->quit();
    thread->wait();
...
HttpLoaderActor::HttpLoaderActor(not_null<HttpLoader *> parent,
                                 not_null<QThread *> thread, const QString &url)
    : _parent(parent) {
  _url = url;
  moveToThread(thread);
...
void HttpLoaderActor::start() { sendRequest(); }

void HttpLoaderActor::sendRequest() {
  auto request = QNetworkRequest(_url);
  const auto rangeHeaderValue =
      "bytes=" + QByteArray::number(_parent->alreadySize()) + "-";
  request.setRawHeader("Range", rangeHeaderValue);
  request.setAttribute(QNetworkRequest::HttpPipeliningAllowedAttribute, true);
  _reply.reset(_manager.get(request));
  connect(_reply.get(), &QNetworkReply::downloadProgress, this,
          &HttpLoaderActor::partFinished);
  connect(_reply.get(), &QNetworkReply::errorOccurred, this,
          &HttpLoaderActor::partFailed);
  connect(_reply.get(), &QNetworkReply::metaDataChanged, this,
...
void HttpLoaderActor::gotMetaData() {
  ...
  for (const auto &pair : pairs) {
    if (QString::fromUtf8(pair.first).toLower() == "content-range") {
      const auto m = QRegularExpression(u"/(\\d+)([^\\d]|$)"_q)
                         .match(QString::fromUtf8(pair.second));
      if (m.hasMatch()) {
...
MtpChecker::MtpChecker(base::weak_ptr<Main::Session> session, bool testing)
    : Checker(testing), _mtp(session) {}
      ...
      [=](const MTPInputChannel &channel) {
        _mtp.send(
            MTPmessages_GetHistory(
                MTP_inputPeerChannel(channel.c_inputChannel().vchannel_id(),
...
auto MtpChecker::parseText(const QByteArray &text) const
  ...
  const auto accumulate = [&](uint64 version, bool isAlpha,
    ...
    if (entry == map.constEnd()) {
      LOG(("Update Error: MTP entry not found for version %1.").arg(version));
      return false;
...
auto MtpChecker::validateLatestLocation(uint64 availableVersion,
                                        const FileLocation &location) const
    -> FileLocation {
...
bool UpdaterDisabled() { return UpdaterIsDisabled; }
...
class Updater : public base::has_weak_ptr {
 ...
 private:
  ...
  void check();
  void startImplementation(not_null<Implementation *> which,
                           std::unique_ptr<Checker> checker);
  bool tryLoaders();
  void handleTimeout();
  void checkerDone(not_null<Implementation *> which,
                   std::shared_ptr<Loader> loader);
...
Updater::Updater()
    : _timer([=] { check(); }), _retryTimer([=] { handleTimeout(); }) {
  checking() | rpl::start_with_next([=] { handleChecking(); }, _lifetime);
  progress() | rpl::start_with_next([=] { handleProgress(); }, _lifetime);
  failed() | rpl::start_with_next([=] { handleFailed(); }, _lifetime);
  ready() | rpl::start_with_next([=] { handleReady(); }, _lifetime);
  isLatest() | rpl::start_with_next([=] { handleLatest(); }, _lifetime);
}

rpl::producer<> Updater::checking() const { return _checking.events(); }

rpl::producer<> Updater::isLatest() const { return _isLatest.events(); }

auto Updater::progress() const -> rpl::producer<Progress> {
  return _progress.events();
}

rpl::producer<> Updater::failed() const { return _failed.events(); }

rpl::producer<> Updater::ready() const { return _ready.events(); }

void Updater::check() { start(false); }
...
void Updater::handleFailed() { scheduleNext(); }
...
void Updater::handleProgress() { _retryTimer.callOnce(kUpdaterTimeout); }
...
void Updater::start(bool forceWait) {
  ...
  const auto updateInSecs = cLastUpdateCheck() + constDelay +
                            int(rand() % randDelay) - base::unixtime::now();
  auto sendRequest =
      (updateInSecs <= 0) || (updateInSecs > constDelay + randDelay);
  ...
  if (sendRequest) {
    startImplementation(&_httpImplementation,
                        std::make_unique<HttpChecker>(_testing));
    startImplementation(&_mtpImplementation,
...
void Updater::startImplementation(not_null<Implementation *> which,
                                  std::unique_ptr<Checker> checker) {
  if (!checker) {
  ...
  checker->ready() | rpl::start_with_next(
                         ...
                         checker->lifetime());
  checker->failed() |
      rpl::start_with_next([=] { checkerFail(which); }, checker->lifetime());

  *which = Implementation{std::move(checker)};

  crl::on_main(which->checker.get(), [=] { which->checker->start(); });
}

void Updater::checkerDone(not_null<Implementation *> which,
                          std::shared_ptr<Loader> loader) {
...
bool Updater::tryLoaders() {
  ...
  const auto tryOne = [&](Implementation &which) {
    ...
    if (const auto loader = _activeLoader.get()) {
      ...
      loader->progress() | rpl::start_to_stream(_progress, loader->lifetime());
      loader->ready() |
          rpl::start_with_next(
              [=](QString &&filepath) { finalize(std::move(filepath)); },
              loader->lifetime());
      loader->failed() |
          rpl::start_with_next([=] { _failed.fire({}); }, loader->lifetime());
  ...
  if (_mtpImplementation.failed && _httpImplementation.failed) {
    ...
    tryOne(_usingMtprotoLoader ? _mtpImplementation : _httpImplementation);
...
void Updater::finalize(QString filepath) {
  ...
  crl::async([=] {
    ...
    crl::on_main([=] { GetUpdaterInstance()->unpackDone(ready); });
  });
}
...
UpdateChecker::UpdateChecker() : _updater(GetUpdaterInstance()) {
...
rpl::producer<> UpdateChecker::checking() const { return _updater->checking(); }

rpl::producer<> UpdateChecker::isLatest() const { return _updater->isLatest(); }

auto UpdateChecker::progress() const -> rpl::producer<Progress> {
  return _updater->progress();
}

rpl::producer<> UpdateChecker::failed() const { return _updater->failed(); }

rpl::producer<> UpdateChecker::ready() const { return _updater->ready(); }

void UpdateChecker::start(bool forceWait) { _updater->start(forceWait); }

void UpdateChecker::test() { _updater->test(); }

void UpdateChecker::setMtproto(base::weak_ptr<Main::Session> session) {
  _updater->setMtproto(session);
}

void UpdateChecker::stop() { _updater->stop(); }

auto UpdateChecker::state() const -> State { return _updater->state(); }

int UpdateChecker::already() const { return _updater->already(); }

int UpdateChecker::size() const { return _updater->size(); }
...
bool checkReadyUpdate() {
  QString readyFilePath = cWorkingDir() + u"tupdates/temp/ready"_q,
          readyPath = cWorkingDir() + u"tupdates/temp"_q;
  ...
  QString versionPath = readyPath + u"/tdata/version"_q;
  {
    ...
    if (versionNum == 0x7FFFFFFF) {  // alpha version
      quint64 alphaVersion = 0;
      if (fVersion.read((char *)&alphaVersion, sizeof(quint64)) !=
          sizeof(quint64)) {
        LOG(("Update Error: cant read alpha version from file '%1'")
                .arg(versionPath));
        ClearAll();
        return false;
      }
      if (!cAlphaVersion() || alphaVersion <= cAlphaVersion()) {
        LOG(("Update Error: cant install alpha version %1 having alpha version "
             "%2")
                .arg(alphaVersion)
                .arg(cAlphaVersion()));
        ClearAll();
      ...
      LOG(("Update Error: cant install version %1 having version %2")
              .arg(versionNum)
              .arg(AppVersion));
      ClearAll();
      return false;
  ...
  QString curUpdater =
      (cExeDir() + cExeName() + u"/Contents/Frameworks/Updater"_q);
  QFileInfo updater(
      cWorkingDir() +
...
#ifdef Q_OS_WIN
  if (CopyFile(updater.absoluteFilePath().toStdWString().c_str(),
               curUpdater.toStdWString().c_str(), FALSE) == FALSE) {
    DWORD errorCode = GetLastError();
    if (errorCode == ERROR_ACCESS_DENIED) {  // we are in write-protected dir,
                                             // like Program Files
  ...
  if (!linuxMoveFile(QFile::encodeName(updater.absoluteFilePath()).constData(),
...
void UpdateApplication() {
  if (UpdaterDisabled()) {
    const auto url = [&] {
#ifdef OS_WIN_STORE
      return "https://www.microsoft.com/en-us/store/p/telegram-desktop/"
             "9nztwsqntd0s";
    ...
    cSetAutoUpdate(true);
    const auto window =
        Core::IsAppLaunched() ? Core::App().activePrimaryWindow() : nullptr;
    if (window) {
      if (const auto controller = window->sessionController()) {
        ...
        window->widget()->showSpecialLayer(Box<::Settings::LayerWidget>(window),
                                           anim::type::normal);
      }
...
QString countAlphaVersionSignature(
    uint64 version) {  // duplicated in packer.cpp
  if (cAlphaPrivateKey().isEmpty()) {
    LOG(
        ("Error: Trying to count alpha version signature without alpha private "
         "key!"));
    return QString();
  }

  QByteArray signedData =
      (qstr("TelegramBeta_") + QString::number(version, 16).toLower()).toUtf8();

  static const int32 shaSize = 20, keySize = 128;

  uchar sha1Buffer[shaSize];
  hashSha1(signedData.constData(), signedData.size(),
           sha1Buffer);  // count sha1

  uint32 siglen = 0;

  RSA *prKey = [] {
    const auto bio =
        MakeBIO(const_cast<char *>(cAlphaPrivateKey().constData()), -1);
  ...
  if (RSA_sign(NID_sha1, (const uchar *)(sha1Buffer), shaSize,
               (uchar *)(signature.data()), &siglen,
               prKey) != 1) {  // count signature
  ...
  signature = signature.toBase64(QByteArray::Base64UrlEncoding |
