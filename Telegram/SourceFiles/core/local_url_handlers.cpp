...
bool SetLanguage(Window::SessionController *controller, const Match &match,
                 const QVariant &context) {
  Lang::CurrentCloudManager().switchWithWarning(u"en"_q);
...
bool ResolveSettings(Window::SessionController *controller, const Match &match,
                     const QVariant &context) {
  ...
  if (base.startsWith(u"doc"_q)) {
    ...
    if (document->isVideoFile()) {
      controller->window().openInMediaView(Media::View::OpenRequest(
          ...
          context ? context->topicRootId() : MsgId(0), false, timeMs));
    } else if (document->isSong() || document->isVoiceMessage()) {
      session->settings().setMediaLastPlaybackPosition(documentId, timeMs);
...
bool ShowEditPersonalChannel(Window::SessionController *controller,
  ...
  auto initBox = [=](not_null<PeerListBox *> box) {
    ...
    rawController->chosen() |
        rpl::start_with_next(
            [=](not_null<ChannelData *> channel) { save(channel); },
            box->lifetime());
...
bool ResolveTestChatTheme(Window::SessionController *controller,
  ...
  if (const auto history = controller->activeChatCurrent().history()) {
    ...
    const auto theme = history->owner().cloudThemes().updateThemeFromLink(
        history->peer->themeEmoji(), params);
    if (theme) {
      ...
      if (const auto strong = weak.get()) {
        ...
        strong->showToast(Ui::Toast::Config{
            .text = tr::lng_credits_enough(
                tr::now, lt_link,
                Ui::Text::Link(
                    Ui::Text::Bold(tr::lng_credits_enough_link(tr::now))),
  ...
  if (telegramMeMatch) {
    ...
    if (const auto phoneMatch =
      ...
      if (const auto threadPostMatch = regex_match(
        ...
        added = u"&post="_q + postMatch->captured(1);
      } else if (const auto storyMatch = regex_match(
                     u"^/s/(\\d+)(/?\\?|/?$)"_q, usernameMatch->captured(2))) {
...
bool InternalPassportLink(const QString &url) {
  ...
  const auto usernameValue =
      usernameMatch->hasMatch()
          ? url_parse_params(usernameMatch->captured(1),
...
bool StartUrlRequiresActivate(const QString &url) {
  return Core::App().passcodeLocked() ? true : !InternalPassportLink(url);
}
