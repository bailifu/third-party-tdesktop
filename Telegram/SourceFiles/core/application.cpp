...
Application::Application()
  ...
  passcodeLockChanges() | rpl::start_with_next(
                              [=](bool locked) {
                                _shouldLockAt = 0;
                                if (locked) {
  ...
  settings().windowTitleContentChanges() |
      rpl::start_with_next([=] { updateWindowTitles(); }, _lifetime);

  _domain->activeSessionChanges() |
      rpl::start_with_next(
          [=](Main::Session *session) {
            if (session &&
                !UpdaterDisabled()) {  // #TODO multi someSessionValue
...
void Application::run() {
  ...
  style::ShortAnimationPlaying() | rpl::start_with_next(
                                       [=](bool playing) {
                                         if (playing) {
                                           MTP::details::pause();
                                         } else {
  ...
  appDeactivatedValue() | rpl::start_with_next(
                              [=](bool deactivated) {
                                if (deactivated) {
...
void Application::startSystemDarkModeViewer() {
  ...
  rpl::merge(settings().systemDarkModeChanges() | rpl::to_empty,
      ...
      rpl::start_with_next([=] { checkSystemDarkMode(); }, _lifetime);
}
...
void Application::createTray() {
  ...
  _tray->showFromTrayRequests() |
      rpl::start_with_next([=] { activate(); }, _lifetime);

  _tray->hideToTrayRequests() | rpl::start_with_next(
                                    [=] {
    ...
    case QEvent::FileOpen: {
      if (object == QCoreApplication::instance()) {
        ...
        if (const auto file = event->file(); !file.isEmpty()) {
          ...
          const auto url =
              QString::fromUtf8(event->url().toEncoded().trimmed());
...
void Application::badMtprotoConfigurationError() {
  if (settings().proxy().isEnabled() && !_badProxyDisableBox) {
    ...
    _badProxyDisableBox->boxClosing() |
        rpl::start_with_next(disableCallback, _badProxyDisableBox->lifetime());
  }
}
...
void Application::startLocalStorage() {
  ...
  settings().saveDelayedRequests() |
      rpl::start_with_next([=] { saveSettingsDelayed(); }, _lifetime);
}
...
void Application::startEmojiImageLoader() {
  ...
  settings().largeEmojiChanges() | rpl::start_with_next(
                                       [=](bool large) {
                                         if (large) {
...
void Application::checkSendPaths() {
  if (!cSendPaths().isEmpty() && _lastActivePrimaryWindow &&
...
QString Application::changelogLink() const {
  ...
  const auto languages = {
      "id", "de", "fr", "nl", "pl",    "tr", "uk", "fa", "ru",
      "ms", "es", "it", "uz", "pt-br", "be", "ar", "ko",
  };
...
Window::Controller *Application::windowForShowingForum(
  ...
  const auto id =
      Window::SeparateId(Window::SeparateType::Forum, forum->history());
  if (const auto separate = separateWindowFor(id)) {
    return separate;
  }
  auto result = (Window::Controller *)nullptr;
