...
void ComputeInstallationTag() {
  ...
  if (file.open(QIODevice::ReadOnly)) {
    const auto result =
        file.read(reinterpret_cast<char *>(&InstallationTag), sizeof(uint64));
...
void Launcher::init() {
  ...
  // but it produces bugs like
  // https://github.com/telegramdesktop/tdesktop/issues/5022 and
  // https://github.com/telegramdesktop/tdesktop/issues/7549 and
  // https://github.com/telegramdesktop/tdesktop/issues/948 more info:
...
void Launcher::prepareSettings() {
  ...
  if (cExeName().isEmpty()) {
    LOG(
        ("WARNING: Could not compute executable path, some features will be "
         "disabled."));
  }
...
void Launcher::processArguments() {
  ...
  gKeyFile = parseResult.value("-key", {})
                 .join(QString())
                 .toLower()
                 .replace(RegExp, {});
  ...
  if (scaleKey.size() > 0) {
    ...
    gConfigScale =
        ((value < kScaleMin) || (value > kScaleMax)) ? kScaleAuto : value;
  }
}
...
void Launcher::checkAndSetLanguage() {
  ...
  language.id = kDefaultLanguage.utf16();
