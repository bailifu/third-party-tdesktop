...
wstring updaterName, updaterDir, updateTo, exeName, customWorkingDir,
    customKeyFile;
...
void openLog() {
  ...
  _logFile = CreateFile(logName, GENERIC_WRITE,
...
void writeLog(const wstring &msg) {
  ...
  wstring full = msg + L'\n';
  DWORD written = 0;
  BOOL result = WriteFile(_logFile, full.c_str(), full.size() * sizeof(wchar_t),
                          &written, 0);
  if (!result) {
    updateError((L"Failed to write log entry '" + msg + L"'").c_str(),
                GetLastError());
    closeLog();
    return;
  }
  ...
  if (!flushr) {
    updateError((L"Failed to flush log on entry '" + msg + L"'").c_str(),
                GetLastError());
    closeLog();
    return;
  }
...
void delFolder() {
  wstring delPathOld = L"tupdates\\ready", delPath = L"tupdates\\temp",
          delFolder = L"tupdates";
...
bool update() {
  ...
  wstring updDir = L"tupdates\\temp", readyFilePath = L"tupdates\\temp\\ready",
          tdataDir = L"tupdates\\temp\\tdata";
  {
    HANDLE readyFile =
        CreateFile(readyFilePath.c_str(), GENERIC_READ, FILE_SHARE_READ, 0,
  ...
  HANDLE versionFile =
      CreateFile((tdataDir + L"\\version").c_str(), GENERIC_READ,
                 FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
  if (versionFile != INVALID_HANDLE_VALUE) {
    if (!ReadFile(versionFile, &versionNum, sizeof(DWORD), &readLen, NULL) ||
        readLen != sizeof(DWORD)) {
      versionNum = 0;
    } else {
      if (versionNum == 0x7FFFFFFF) {  // alpha version

      } else if (!ReadFile(versionFile, &versionLen, sizeof(DWORD), &readLen,
                           NULL) ||
                 readLen != sizeof(DWORD) || versionLen > 63) {
        versionNum = 0;
      } else if (!ReadFile(versionFile, versionStr, versionLen, &readLen,
                           NULL) ||
                 readLen != versionLen) {
        versionNum = 0;
      }
  ...
  do {
    ...
    HANDLE findHandle =
        FindFirstFileEx((dir + L"\\*").c_str(), FindExInfoStandard, &findData,
    ...
    do {
      ...
      if (fname.substr(0, tdataDir.size()) == tdataDir &&
          (fname.size() <= tdataDir.size() ||
           fname.at(tdataDir.size()) == '/')) {
        writeLog(L"Skipped 'tdata' path '" + fname + L"'");
      } else if (findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
        if (findData.cFileName != wstring(L".") &&
            findData.cFileName != wstring(L"..")) {
          dirs.push_back(fname);
          writeLog(L"Added dir '" + fname + L"' in update tree..");
        }
      } else {
        wstring tofname = updateTo + fname.substr(updDir.size() + 1);
        if (equal(tofname, updaterName)) {  // bad update - has Updater.exe -
                                            // delete all dir
          writeLog(L"Error: bad update, has Updater.exe! '" + tofname +
                   L"' equal '" + updaterName + L"'");
          delFolder();
          return false;
        } else if (equal(tofname, updateTo + L"Telegram.exe") &&
                   exeName != L"Telegram.exe") {
          wstring fullBinaryPath = updateTo + exeName;
          writeLog(L"Target binary found: '" + tofname + L"', changing to '" +
                   fullBinaryPath + L"'");
        ...
        if (equal(fname, readyFilePath)) {
          ...
          writeLog(L"Added file '" + fname + L"' to be copied to '" + tofname +
                   L"'");
        }
      }
    ...
    do {
      ...
      if (!copyResult) {
        ...
        wsprintf(errMsg, L"Failed to update Telegram :(\n%s is not accessible.",
                 tofname.c_str());
        if (MessageBox(0, errMsg, L"Update error!",
  ...
  if (versionNum && versionNum != 0x7FFFFFFF) {
    ...
    LSTATUS status =
        RegOpenKeyEx(HKEY_CURRENT_USER,
                     ...
                     L"\\{53F49750-6209-4FBF-9CA8-7A333C87D1ED}_is1",
                     0, KEY_QUERY_VALUE | KEY_SET_VALUE, &rkey);
    if (status == ERROR_SUCCESS) {
      ...
      if (RegQueryValueEx(rkey, L"InstallLocation", 0, &locationType,
                          (BYTE *)locationStr,
                          &locationSize) == ERROR_SUCCESS) {
        locationSize /= 2;
        ...
        if (locationType == REG_EXPAND_SZ || locationType == REG_SZ) {
          if (PathCanonicalize(exp, locationStr)) {
            ...
            if (GetFullPathName(L".", bufSize, exp, 0) < bufSize) {
              ...
              if (installpath == mypath + L"\\" ||
                  true) {  // always update reg info, if we found it
                WCHAR nameStr[bufSize], dateStr[bufSize], publisherStr[bufSize],
                    icongroupStr[bufSize];
                SYSTEMTIME stLocalTime;
                GetLocalTime(&stLocalTime);
                RegSetValueEx(rkey, L"DisplayVersion", 0, REG_SZ,
                              (const BYTE *)versionStr,
                              ((versionLen / 2) + 1) * sizeof(WCHAR));
                wsprintf(nameStr, L"Telegram Desktop");
                RegSetValueEx(rkey, L"DisplayName", 0, REG_SZ,
                              (const BYTE *)nameStr,
                              (wcslen(nameStr) + 1) * sizeof(WCHAR));
                wsprintf(publisherStr, L"Telegram FZ-LLC");
                RegSetValueEx(rkey, L"Publisher", 0, REG_SZ,
                              (const BYTE *)publisherStr,
                              (wcslen(publisherStr) + 1) * sizeof(WCHAR));
                wsprintf(icongroupStr, L"Telegram Desktop");
                RegSetValueEx(rkey, L"Inno Setup: Icon Group", 0, REG_SZ,
                              (const BYTE *)icongroupStr,
                              (wcslen(icongroupStr) + 1) * sizeof(WCHAR));
                wsprintf(dateStr, L"%04d%02d%02d", stLocalTime.wYear,
                         stLocalTime.wMonth, stLocalTime.wDay);
                RegSetValueEx(rkey, L"InstallDate", 0, REG_SZ,
                              (const BYTE *)dateStr,
                              (wcslen(dateStr) + 1) * sizeof(WCHAR));

                const WCHAR *appURL = L"https://baidu.com/";
                RegSetValueEx(rkey, L"HelpLink", 0, REG_SZ,
                              (const BYTE *)appURL,
                              (wcslen(appURL) + 1) * sizeof(WCHAR));
                RegSetValueEx(rkey, L"URLInfoAbout", 0, REG_SZ,
                              (const BYTE *)appURL,
                              (wcslen(appURL) + 1) * sizeof(WCHAR));
                RegSetValueEx(rkey, L"URLUpdateInfo", 0, REG_SZ,
                              (const BYTE *)appURL,
                              (wcslen(appURL) + 1) * sizeof(WCHAR));
              }
            }
...
int APIENTRY wWinMain(HINSTANCE instance, HINSTANCE prevInstance,
                      LPWSTR cmdParamarg, int cmdShow) {
  base::Platform::InitDynamicLibraries();

  openLog();

  _oldWndExceptionFilter = SetUnhandledExceptionFilter(_exceptionFilter);
  //	CAPIHook apiHook("kernel32.dll", "SetUnhandledExceptionFilter",
  ...
  bool needupdate = false, autostart = false, debug = false,
       writeprotected = false, startintray = false;
  ...
  if (args) {
    ...
    if (!customWorkingDir.empty())
      writeLog(L"Will pass custom working dir: " + customWorkingDir);
    ...
    if (updaterName.size() > 11) {
      if (equal(updaterName.substr(updaterName.size() - 11), L"Updater.exe")) {
        ...
        if (writeprotected) {  // if we can't clear all tupdates\ready
                               // (Updater.exe is there) - clear only version
  ...
  if (!executed) {
    ShellExecute(0, 0, (updateTo + exeName).c_str(),
                 (L"-noupdate" + targs).c_str(), 0, SW_SHOWNORMAL);
  }
...
static const WCHAR *_programName =
    L"Telegram Desktop";  // folder in APPDATA, if current path is unavailable
                          // for writing
static const WCHAR *_exeName = L"Updat3r.exe";

LPTOP_LEVEL_EXCEPTION_FILTER _oldWndExceptionFilter = 0;

typedef BOOL(FAR STDAPICALLTYPE *t_miniDumpWriteDump)(
    _In_ HANDLE hProcess, _In_ DWORD ProcessId, _In_ HANDLE hFile,
...
HANDLE _generateDumpFileAtPath(const WCHAR *path) {
  ...
  wsprintf(szFileName, L"%s%s-%s-%04d%02d%02d-%02d%02d%02d-%ld-%ld.dmp", szPath,
           szExeName, updaterVersionStr, stLocalTime.wYear, stLocalTime.wMonth,
           stLocalTime.wDay, stLocalTime.wHour, stLocalTime.wMinute,
           stLocalTime.wSecond, GetCurrentProcessId(), GetCurrentThreadId());
  return CreateFile(szFileName, GENERIC_READ | GENERIC_WRITE,
...
void _generateDump(EXCEPTION_POINTERS *pExceptionPointers) {
  ...
  miniDumpWriteDump =
      (t_miniDumpWriteDump)GetProcAddress(hDll, "MiniDumpWriteDump");
  ...
  if (!hDumpFile || hDumpFile == INVALID_HANDLE_VALUE) {
    ...
    DWORD wstrPathLen =
        GetEnvironmentVariable(L"APPDATA", wstrPath, maxFileLen);
    if (wstrPathLen) {
...
LONG CALLBACK _exceptionFilter(EXCEPTION_POINTERS *pExceptionPointers) {
  ...
  return _oldWndExceptionFilter ? (*_oldWndExceptionFilter)(pExceptionPointers)
