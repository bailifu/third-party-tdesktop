...
ConnectionState::Widget::ProxyIcon::ProxyIcon(QWidget *parent)
    : RpWidget(parent) {
  resize(std::max(st::connectingRadial.size.width(),
                  st::connectingProxyOn.width()),
         std::max(st::connectingRadial.size.height(),
                  st::connectingProxyOn.height()));

  style::PaletteChanged() |
      rpl::start_with_next([=] { refreshCacheImages(); }, lifetime());
...
void ConnectionState::Widget::ProxyIcon::refreshCacheImages() {
  const auto prepareCache = [&](const style::icon &icon) {
    auto image = QImage(size() * style::DevicePixelRatio(),
                        QImage::Format_ARGB32_Premultiplied);
    image.setDevicePixelRatio(style::DevicePixelRatio());
    image.fill(st::windowBg->c);
    {
      auto p = QPainter(&image);
      icon.paint(p, (width() - icon.width()) / 2,
                 (height() - icon.height()) / 2, width());
    }
...
bool ConnectionState::State::operator==(const State &other) const {
  return (type == other.type) && (useProxy == other.useProxy) &&
         (underCursor == other.underCursor) &&
         (updateReady == other.updateReady) &&
         (waitTillRetry == other.waitTillRetry);
}

ConnectionState::ConnectionState(not_null<Ui::RpWidget *> parent,
  ...
  if (!Core::UpdaterDisabled()) {
    ...
    rpl::merge(rpl::single(rpl::empty), checker.ready()) |
        rpl::start_with_next([=] { refreshState(); }, _lifetime);
  }

  rpl::combine(Core::App().settings().proxy().connectionTypeValue(),
...
void ConnectionState::setBottomSkip(int skip) { _bottomSkip = skip; }
...
void ConnectionState::refreshState() {
  ...
  const auto state = [&]() -> State {
    ...
    if (state == MTP::ConnectingState || state == MTP::DisconnectedState ||
        (state < 0 && state > -600)) {
      return {State::Type::Connecting, proxy, exposed, under, ready};
    } else if (state < 0 && state >= -kMinimalWaitingStateDuration &&
  ...
  if (state == _state) {
    ...
    const auto applyConnectingAt =
        std::max(_connectingStartedAt + kConnectingStateDelay,
...
void ConnectionState::applyState(const State &state) {
  ...
  if (_currentLayout.contentWidth != newLayout.contentWidth) {
    if (!_currentLayout.contentWidth || !newLayout.contentWidth ||
      ...
      _contentWidth.start([=] { updateWidth(); }, _currentLayout.contentWidth,
                          newLayout.contentWidth, st::connectingDuration);
    }
  }
  const auto saved = _currentLayout;
  setLayout(newLayout);
  if (_currentLayout.text.isEmpty() && !saved.text.isEmpty() &&
      _contentWidth.animating()) {
...
void ConnectionState::changeVisibilityWithLayout(const Layout &layout) {
  ...
  _visibility.start([=] { updateVisibility(); }, layout.visible ? 0. : 1.,
                    layout.visible ? 1. : 0., st::connectingDuration);
  if (_contentWidth.animating()) {
    _contentWidth.start([=] { updateWidth(); }, _currentLayout.contentWidth,
...
auto ConnectionState::computeLayout(const State &state) const -> Layout {
  ...
  result.visible = state.exposed && !state.updateReady &&
                   (state.useProxy || state.type == State::Type::Connecting ||
  ...
  switch (state.type) {
    ...
    case State::Type::Waiting:
      ...
      result.text =
          tr::lng_reconnecting(tr::now, lt_count, state.waitTillRetry);
      break;
  }
  result.textWidth = st::normalFont->width(result.text);
  result.contentWidth =
      (result.textWidth > 0)
          ? (st::connectingTextPadding.left() + result.textWidth +
...
void ConnectionState::updateWidth() {
  ...
  const auto desired =
      QRect(0, 0, current, height)
          .marginsAdded(style::margins(st::connectingLeft.width(), 0,
                                       st::connectingRight.width(), 0))
          .marginsAdded(st::connectingMargin);
  if (_widget) {
...
QRect ConnectionState::Widget::innerRect() const {
  return rect().marginsRemoved(st::connectingMargin);
}

QRect ConnectionState::Widget::contentRect() const {
  return innerRect().marginsRemoved(style::margins(
      st::connectingLeft.width(), 0, st::connectingRight.width(), 0));
}

QRect ConnectionState::Widget::textRect() const {
  return contentRect().marginsRemoved(st::connectingTextPadding);
}
